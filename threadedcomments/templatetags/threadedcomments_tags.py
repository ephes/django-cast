...
class BaseThreadedCommentNode(AdminOverrideCommentNode):
    ...
    def handle_token(cls, parser, token):
        ...
        if len(tokens) == 5:
            ...
            return cls(object_expr=parser.compile_filter(tokens[2]), as_varname=tokens[4], **extra_kw)
        elif len(tokens) == 6:
    ...
    def get_queryset(self, context):
        ...
        if self.limit:
            parent_qs = (
                qs.exclude(parent__isnull=False)
                .order_by("-newest_activity", "-submit_date")
        ...
        if len(tokens) < 7:
            ...
            if tokens[-2] != "with":
                raise template.TemplateSyntaxError(
                    f"{tokens[0]!r} tag must have a 'with' as the last but one argument."
                )
            return cls(
...
class RenderCommentFormNode(CommentFormNode):
    ...
    def handle_token(cls, parser, token):
        ...
        if len(tokens) == 3:
            ...
            return cls(object_expr=parser.compile_filter(tokens[2]), parent=parser.compile_filter(tokens[4]))
        elif len(tokens) == 6:
