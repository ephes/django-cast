...
def index(request):
    snippet_model_opts = [
        model._meta for model in get_snippet_models() if user_can_edit_snippet_type(request.user, model)
    ]
    if snippet_model_opts:
        return TemplateResponse(
            request,
            "wagtailsnippets/snippets/index.html",
            {"snippet_model_opts": sorted(snippet_model_opts, key=lambda x: x.verbose_name.lower())},
        )
    else:
...
class ListView(TemplateView):
    ...
    def get(self, request, app_label, model_name):
        ...
        permissions = [get_permission_name(action, self.model) for action in ["add", "change", "delete"]]
    ...
    def get_context_data(self, **kwargs):
        ...
        enable_locale_filter = getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(
            ...
            context.update(
                {
                    ...
                    "translations": [
                        {
                            "locale": locale,
                            "url": reverse("wagtailsnippets:list", args=[self.app_label, self.model_name])
                            + "?locale="
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                buttons=[
                    messages.button(
                        reverse("wagtailsnippets:edit", args=(app_label, model_name, quote(instance.pk))), _("Edit")
                    )
                ],
            )
            ...
            messages.validation_error(request, _("The snippet could not be created due to errors."), form)
    else:
        ...
        context.update(
            {
                ...
                "translations": [
                    {
                        "locale": locale,
                        "url": reverse("wagtailsnippets:add", args=[app_label, model_name])
                        + "?locale="
                        + locale.language_code,
                    }
...
def edit(request, app_label, model_name, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.validation_error(request, _("The snippet could not be saved due to errors."), form)
    else:
        form = form_class(instance=instance)
    ...
    if request.method == "POST":
        ...
        if count == 1:
            ...
            message_content = ngettext(
                "%(count)d %(snippet_type)s deleted.", "%(count)d %(snippet_type)s deleted.", count
...
def usage(request, app_label, model_name, pk):
    ...
    return TemplateResponse(request, "wagtailsnippets/snippets/usage.html", {"instance": instance, "used_by": used_by})
