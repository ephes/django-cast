    ...
    def get(self, request, app_label, model_name):
        ...
        if self.is_searchable and "q" in request.GET:
            self.search_form = SearchForm(
                request.GET,
                placeholder=_("Search %(snippet_type_name)s") % {"snippet_type_name": self.model._meta.verbose_name},
            )
...
def chosen(request, app_label, model_name, pk):
    ...
    snippet_data = {
        "id": str(item.pk),
        "string": str(item),
        "edit_link": reverse("wagtailsnippets:edit", args=(app_label, model_name, quote(item.pk))),
    }
