...
class TestSnippetListView(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(reverse("wagtailsnippets:list", args=("tests", "advert")), params)
...
class TestLocaleSelectorOnList(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector(self):
        response = self.client.get(reverse("wagtailsnippets:list", args=["snippetstests", "translatablesnippet"]))

        switch_to_french_url = (
            reverse("wagtailsnippets:list", args=["snippetstests", "translatablesnippet"]) + "?locale=fr"
        )
        ...
        self.assertContains(
            response, f'No translatable snippets have been created. Why not <a href="{add_url}">add one</a>'
        )
    ...
    def test_locale_selector_not_present_when_i18n_disabled(self):
        ...
        self.assertNotContains(
            response, f'<a href="{switch_to_french_url}" aria-label="French" class="u-link is-live">'
        )
...
class TestModelOrdering(TestCase, WagtailTestUtils):
    ...
    def test_listing_respects_model_ordering(self):
        response = self.client.get(reverse("wagtailsnippets:list", args=("tests", "advertwithtabbedinterface")))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["items"][0].text, "aaaadvert")

    def test_chooser_respects_model_ordering(self):
        response = self.client.get(reverse("wagtailsnippets:choose", args=("tests", "advertwithtabbedinterface")))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["items"][0].text, "aaaadvert")
...
class TestSnippetListViewWithSearchableSnippet(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(reverse("wagtailsnippets:list", args=("snippetstests", "searchablesnippet")), params)
...
class TestSnippetCreateView(TestCase, WagtailTestUtils):
    ...
    def test_simple(self):
        ...
        self.assertNotContains(
            response, '<a href="#tab-advert" class="active" data-tab="advert">Advert</a>', html=True
        )
    ...
    def test_snippet_with_tabbed_interface(self):
        response = self.client.get(reverse("wagtailsnippets:add", args=("tests", "advertwithtabbedinterface")))
    ...
    def test_create_invalid(self):
        ...
        self.assertContains(
            response, """<p class="error-message"><span>This field is required.</span></p>""", count=1, html=True
        )
    ...
    def test_create_file_upload_multipart(self):
        ...
        response = self.post(
            model=FileUploadSnippet, post_data={"file": SimpleUploadedFile("test.txt", b"Uploaded file")}
        )
        self.assertRedirects(response, reverse("wagtailsnippets:list", args=("snippetstests", "fileuploadsnippet")))
...
class TestLocaleSelectorOnCreate(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector(self):
        response = self.client.get(reverse("wagtailsnippets:add", args=["snippetstests", "translatablesnippet"]))

        switch_to_french_url = (
            reverse("wagtailsnippets:add", args=["snippetstests", "translatablesnippet"]) + "?locale=fr"
        )
        self.assertContains(response, f'<a href="{switch_to_french_url}" aria-label="French" class="u-link is-live">')
    ...
    def test_locale_selector_not_present_on_non_translatable_snippet(self):
        response = self.client.get(reverse("wagtailsnippets:add", args=["tests", "advert"]))
...
class TestSnippetEditView(BaseTestSnippetEditView):
    ...
    def test_edit_with_tags(self):
        tags = ["hello", "world"]
        response = self.post(
            post_data={"text": "edited_test_advert", "url": "http://www.example.com/edited", "tags": ", ".join(tags)}
        )
    ...
    def test_before_edit_snippet_hook_post(self):
        ...
        with self.register_hook("before_edit_snippet", hook_func):
            response = self.post(
                post_data={"text": "Edited and runs hook", "url": "http://www.example.com/hook-enabled-edited"}
            )
...
class TestEditTabbedSnippet(BaseTestSnippetEditView):
    def setUp(self):
        ...
        self.test_snippet = AdvertWithTabbedInterface.objects.create(
            text="test_advert", url="http://www.example.com", something_else="Model with tabbed interface"
        )
...
class TestLocaleSelectorOnEdit(BaseTestSnippetEditView):
    ...
    LOCALE_SELECTOR_HTML = (
        '<a href="javascript:void(0)" aria-label="English" class="c-dropdown__button  u-btn-current">'
    )
    ...
    def test_locale_selector(self):
        ...
        switch_to_french_url = reverse(
            "wagtailsnippets:edit", args=["snippetstests", "translatablesnippet", quote(self.test_snippet_fr.pk)]
        )
        self.assertContains(response, f'<a href="{switch_to_french_url}" aria-label="French" class="u-link is-live">')
...
class TestSnippetChooserPanel(TestCase, WagtailTestUtils):
    ...
    def setUp(self):
        ...
        edit_handler = self.edit_handler.bind_to(instance=test_snippet, form=form, request=self.request)
    ...
    def test_render_js(self):
        self.assertIn('createSnippetChooser("id_advert");', self.snippet_chooser_panel.render_as_field())

    def test_target_model_autodetected(self):
        result = SnippetChooserPanel("advert").bind_to(model=SnippetChooserModel).target_model
...
class TestSnippetChoose(TestCase, WagtailTestUtils):
    ...
    def get(self, params=None):
        return self.client.get(reverse("wagtailsnippets:choose", args=self.url_args), params or {})
...
class TestSnippetChooseWithSearchableSnippet(TestCase, WagtailTestUtils):
    ...
    def get(self, params=None):
        return self.client.get(
            reverse("wagtailsnippets:choose", args=("snippetstests", "searchablesnippet")), params or {}
        )
...
class TestSnippetChosen(TestCase, WagtailTestUtils):
    ...
    def get(self, pk, params=None):
        return self.client.get(reverse("wagtailsnippets:chosen", args=("tests", "advert", pk)), params or {})
...
class TestAddOnlyPermissions(TestCase, WagtailTestUtils):
    ...
    def test_get_edit(self):
        response = self.client.get(
            reverse("wagtailsnippets:edit", args=("tests", "advert", quote(self.test_snippet.pk)))
        )
...
class TestInlinePanelMedia(TestCase, WagtailTestUtils):
    ...
    def test_inline_panel_media(self):
        ...
        response = self.client.get(
            reverse("wagtailsnippets:add", args=("snippetstests", "multisectionrichtextsnippet"))
        )
        self.assertEqual(response.status_code, 200)
...
class TestSnippetListViewWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("wagtailsnippets:list", args=("snippetstests", "standardsnippetwithcustomprimarykey")), params
        )
...
class TestSnippetViewWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def test_edit(self):
        ...
        self.assertRedirects(
            response, reverse("wagtailsnippets:list", args=("snippetstests", "standardsnippetwithcustomprimarykey"))
        )
...
class TestSnippetChooserBlockWithCustomPrimaryKey(TestCase):
    ...
    def test_form_response(self):
        ...
        value = block.value_from_datadict(
            {"advertwithcustomprimarykey": str(test_advert.pk)}, {}, "advertwithcustomprimarykey"
        )
        self.assertEqual(value, test_advert)
...
class TestSnippetChooserPanelWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def test_render_js(self):
        self.assertIn(
            'createSnippetChooser("id_advertwithcustomprimarykey");', self.snippet_chooser_panel.render_as_field()
        )
...
class TestSnippetChooseWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, params=None):
        return self.client.get(
            reverse("wagtailsnippets:choose", args=("tests", "advertwithcustomprimarykey")), params or {}
        )
...
class TestSnippetChosenWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, pk, params=None):
        return self.client.get(
            reverse("wagtailsnippets:chosen", args=("tests", "advertwithcustomprimarykey", quote(pk))), params or {}
        )
...
class TestSnippetChosenWithCustomUUIDPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, pk, params=None):
        return self.client.get(
            reverse("wagtailsnippets:chosen", args=("tests", "advertwithcustomuuidprimarykey", quote(pk))),
            params or {},
        )
