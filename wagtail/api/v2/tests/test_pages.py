...
class TestPageListing(TestCase):
    ...
    def test_basic(self):
        ...
        for page in content["items"]:
            ...
            self.assertEqual(
                set(page["meta"].keys()), {"type", "detail_url", "html_url", "slug", "first_published_at"}
            )
    ...
    def test_fields_foreign_key(self):
        ...
        for page in content["items"]:
            ...
            if feed_image is not None:
                ...
                self.assertEqual(
                    feed_image["meta"]["detail_url"], "http://localhost/api/main/images/%d/" % feed_image["id"]
                )
    ...
    def test_filtering_unknown_field_gives_error(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            content, {"message": "query parameter is not an operation or a recognised field: not_a_field"}
        )
