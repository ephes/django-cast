...
class SearchFilter(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        if "search" in request.GET:
            ...
            try:
                queryset = sb.search(
                    search_query, queryset, operator=search_operator, order_by_relevance=order_by_relevance
                )
            except FilterFieldError as e:
                raise BadRequestError(f"cannot filter by '{e.field_name}' while searching (field is not indexed)")
            except OrderByFieldError as e:
                raise BadRequestError(f"cannot order by '{e.field_name}' while searching (field is not indexed)")
