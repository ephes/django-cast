...
class TestChooser(TestCase, WagtailTestUtils):
    ...
    def test_submit_valid_embed(self, get_embed):
        ...
        response = self.client.post(
            reverse("wagtailembeds:chooser_upload"), {"embed-chooser-url": "http://www.example.com/"}
        )
...
class TestEmbedly(TestCase):
    ...
    def test_embedly_401(self):
        with patch("embedly.Embedly.oembed") as oembed:
            ...
            self.assertRaises(
                AccessDeniedEmbedlyException, EmbedlyFinder(key="foo").find_embed, "http://www.example.com"
            )
...
class TestOembed(TestCase):
    ...
    def test_oembed_non_json_response(self, urlopen):
        ...
        self.assertRaises(
            EmbedNotFoundException, OEmbedFinder().find_embed, "https://www.youtube.com/watch?v=ReblZ7o7lu4"
        )
...
class TestInstagramOEmbed(TestCase):
    ...
    def test_instagram_request_not_found(self):
        ...
        with patch.object(urllib.request, "urlopen", **config):
            self.assertRaises(
                EmbedNotFoundException, InstagramOEmbedFinder().find_embed, "https://instagr.am/p/CHeRxmnDSYe/"
            )
...
class TestFacebookOEmbed(TestCase):
    ...
    def test_facebook_request_denied_401(self):
        ...
        with patch.object(urllib.request, "urlopen", **config):
            self.assertRaises(
                AccessDeniedFacebookOEmbedException, FacebookOEmbedFinder().find_embed, "https://fb.watch/ABC123eew/"
            )
