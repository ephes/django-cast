...
def widget_with_script(widget, script):
    return mark_safe(f"{widget}<script>{script}</script>")
...
def get_form_for_model(
    ...
    form_class_attrs = {"Meta": type("Meta", bases, attrs)}
...
class EditHandler:
    ...
    def __repr__(self):
        return "<{} with model={} instance={} request={} form={}>".format(
            self.__class__.__name__, self.model, self.instance, self.request, self.form.__class__.__name__
        )
...
class BaseCompositeEditHandler(EditHandler):
    ...
    def on_instance_bound(self):
        self.children = [child.bind_to(instance=self.instance) for child in self.children]
...
class TabbedInterface(BaseFormEditHandler):
    ...
    def get_form_class(self):
        ...
        return type(form_class.__name__, (form_class,), {"show_comments_toggle": self.show_comments_toggle})
...
class HelpPanel(EditHandler):
    def __init__(self, content="", template="wagtailadmin/edit_handlers/help_panel.html", heading="", classname=""):
            ...
            for page_type in self.page_type:
                try:
                    ...
                    raise ImproperlyConfigured(
                        "{}.page_type must be of the form 'app_label.model_name', given {!r}".format(
                            self.__class__.__name__, page_type
                        )
                    )
                except ValueError:
                    raise ImproperlyConfigured(
                        "{}.page_type refers to model {!r} that has not been installed".format(
                            self.__class__.__name__, page_type
                        )
                    )
...
class InlinePanel(EditHandler):
    ...
    def get_comparison(self):
        ...
        for panel in self.get_panel_definitions():
            field_comparisons.extend(panel.bind_to(model=self.db_field.related_model).get_comparison())
    ...
    def on_form_bound(self):
        ...
        for subform in self.formset.forms:
            ...
            self.children.append(
                child_edit_handler.bind_to(instance=subform.instance, request=self.request, form=subform)
            )
...
def get_edit_handler(cls):
    ...
    if hasattr(cls, "edit_handler"):
        ...
        if cls.settings_panels:
            tabs.append(ObjectList(cls.settings_panels, heading=gettext_lazy("Settings"), classname="settings"))
