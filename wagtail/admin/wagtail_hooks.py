...
from wagtail.admin.rich_text import HalloFormatPlugin, HalloHeadingPlugin, HalloListPlugin, HalloPlugin
from wagtail.admin.rich_text.converters.contentstate import link_entity
from wagtail.admin.rich_text.converters.editor_html import LinkTypeRule, PageLinkHandler, WhitelistRule
    ...
    def render_component(self, request):
        ...
        if start_page:
            return PageExplorerMenuItemComponent(
                self.name, self.label, self.url, start_page.id, icon_name=self.icon_name, classnames=self.classnames
            )
        else:
...
def register_explorer_menu_item():
    return ExplorerMenuItem(
        _("Pages"), reverse("wagtailadmin_explore_root"), name="explorer", icon_name="folder-open-inverse", order=100
    )
...
class PageSearchArea(SearchArea):
    def __init__(self):
        super().__init__(
            _("Pages"), reverse("wagtailadmin_pages:search"), name="pages", icon_name="folder-open-inverse", order=100
        )
...
def register_collections_menu_item():
    return CollectionsMenuItem(
        _("Collections"), reverse("wagtailadmin_collections:index"), icon_name="folder-open-1", order=700
    )
...
def register_workflow_tasks_menu_item():
    return WorkflowTasksMenuItem(
        _("Workflow tasks"), reverse("wagtailadmin_workflows:task_index"), icon_name="thumbtack", order=150
    )
...
def register_core_features(features):
    ...
    features.register_converter_rule("editorhtml", "hr", [WhitelistRule("hr", allow_without_attributes)])
    ...
    features.register_editor_plugin("hallo", "bold", HalloFormatPlugin(format_name="bold"))
    features.register_converter_rule(
        "editorhtml",
        "bold",
        [
    ...
    features.register_editor_plugin("hallo", "italic", HalloFormatPlugin(format_name="italic"))
    features.register_converter_rule(
        "editorhtml",
        "italic",
        [
    ...
    for order, element in enumerate(headings_elements, start=headings_order_start):
        features.register_editor_plugin("hallo", element, HalloHeadingPlugin(element=element, order=order))
    ...
    features.register_editor_plugin("hallo", "ul", HalloListPlugin(list_type="unordered"))
    features.register_converter_rule(
        "editorhtml",
        "ul",
        [
...
def register_locked_pages_menu_item():
    return LockedPagesMenuItem(
        _("Locked Pages"), reverse("wagtailadmin_reports:locked_pages"), icon_name="lock", order=700
    )
...
def register_workflow_report_menu_item():
    return WorkflowReportMenuItem(
        _("Workflows"), reverse("wagtailadmin_reports:workflow"), icon_name="tasks", order=800
    )
...
def register_workflow_tasks_report_menu_item():
    return WorkflowReportMenuItem(
        _("Workflow tasks"), reverse("wagtailadmin_reports:workflow_tasks"), icon_name="thumbtack", order=900
    )
...
def register_site_history_report_menu_item():
    return SiteHistoryReportMenuItem(
        _("Site history"), reverse("wagtailadmin_reports:site_history"), icon_name="history", order=1000
    )
