    ...
    def get_context_data(self, parent_context):
        ...
        if getattr(settings, "WAGTAIL_WORKFLOW_ENABLED", True):
            ...
            context["states"] = [
                (
                    state,
                    state.task.specific.get_actions(page=state.page_revision.page, user=request.user),
...
class RecentEditsPanel(Component):
    ...
    def get_context_data(self, parent_context):
        ...
        if connection.vendor == "mysql":
            ...
            last_edits = PageRevision.objects.raw(
                ...
                SELECT wp.* FROM
                 ...
                 """,
                [User._meta.pk.get_db_prep_value(request.user.pk, connection), edit_count],
            )
        else:
            last_edits_dates = (
        ...
        context["last_edits"] = [[revision, pages.get(revision.page_id)] for revision in last_edits]
        context["request"] = request
        return context
    ...
    for panel in panels:
        if hasattr(panel, "render") and not hasattr(panel, "render_html"):
            ...
            message = (
                ...
                "See https://docs.wagtail.io/en/stable/releases/2.15.html#template-components-2-15" % panel
            )
