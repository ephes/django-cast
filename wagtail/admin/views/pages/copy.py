            ...
            if form.cleaned_data.get("copy_subpages"):
                messages.success(
                    ...
                    _("Page '{0}' and {1} subpages copied.").format(
                        page.specific_deferred.get_admin_display_title(), new_page.get_descendants().count()
                    ),
                )
            else:
                messages.success(
                    request, _("Page '{0}' copied.").format(page.specific_deferred.get_admin_display_title())
                )
