...
class EditView(TemplateResponseMixin, ContextMixin, HookResponseMixin, View):
    ...
    def add_save_confirmation_message(self):
        if self.is_reverting:
            message = _("Page '{0}' has been replaced with version from {1}.").format(
                self.page.get_admin_display_title(), self.previous_revision.created_at.strftime("%d %b %Y %H:%M")
            )
        else:
            message = _("Page '{0}' has been updated.").format(self.page.get_admin_display_title())
    ...
    def send_commenting_notifications(self, changes):
        ...
        subscribers = PageSubscription.objects.filter(page=self.page, comment_notifications=True).select_related(
            "user"
        )
        global_recipient_users = [
            subscriber.user for subscriber in subscribers if subscriber.user != self.request.user
        ]
        ...
        thread_users = (
            ...
            .prefetch_related(Prefetch("comment_replies", queryset=replies), Prefetch("comments", queryset=comments))
            .exclude(Q(comment_replies__isnull=True) & Q(comments__isnull=True))
        )
        ...
        for current_user, current_threads in thread_users:
            ...
            send_notification(
                ...
                "updated_comments",
                {
                    ...
                    "resolved_comments": [
                        comment for comment in changes["resolved_comments"] if comment.pk in threads
                    ],
                    "deleted_comments": [],
                    "replied_comments": [
                        {
    ...
    def get(self, request):
        if self.page_perms.user_has_lock():
            if self.page.locked_at:
                ...
                lock_message = format_html(
                    _("<b>Page '{}' is locked</b> by <b>you</b>."), self.page.get_admin_display_title()
                )
            ...
            if (
                ...
                if len(self.workflow_tasks) == 1:
                    ...
                    workflow_info = format_html(
                        _("This page is awaiting <b>'{}'</b> in the <b>'{}'</b> workflow."), task.name, workflow.name
                    )
                messages.error(
                    self.request,
                    mark_safe(workflow_info + " " + _("Only reviewers for this task can edit the page.")),
                    extra_tags="lock",
                )
    ...
    def add_cancel_workflow_confirmation_message(self):
        message = _("Workflow on page '{0}' has been cancelled.").format(self.page.get_admin_display_title())
    ...
    def publish_action(self):
        ...
        if go_live_at and go_live_at > timezone.now():
            ...
            if self.is_reverting:
                message = _("Version from {0} of page '{1}' has been scheduled for publishing.").format(
                    self.previous_revision.created_at.strftime("%d %b %Y %H:%M"), self.page.get_admin_display_title()
                )
            else:
                if self.page.live:
                    message = _("Page '{0}' is live and this version has been scheduled for publishing.").format(
                        self.page.get_admin_display_title()
                    )
            ...
            if self.is_reverting:
                message = _("Version from {0} of page '{1}' has been published.").format(
                    self.previous_revision.created_at.strftime("%d %b %Y %H:%M"), self.page.get_admin_display_title()
                )
            else:
                message = _("Page '{0}' has been published.").format(self.page.get_admin_display_title())
    ...
    def submit_action(self):
        ...
        message = _("Page '{0}' has been submitted for moderation.").format(self.page.get_admin_display_title())
    ...
    def restart_workflow_action(self):
        ...
        message = _("Workflow on page '{0}' has been restarted.").format(self.page.get_admin_display_title())
    ...
    def perform_workflow_action(self):
        ...
        self.page.current_workflow_task.on_action(
            self.page.current_workflow_task_state, self.request.user, self.workflow_action, **extra_workflow_data
        )
