...
class BaseWorkflowFormView(View):
    ...
    def dispatch(self, request, page_id, action_name, task_state_id):
        ...
        if not self.page.workflow_in_progress:
            messages.error(
                request,
                _("The page '{0}' is not currently awaiting moderation.").format(self.page.get_admin_display_title()),
            )
...
class WorkflowAction(BaseWorkflowFormView):
    ...
    def post(self, request, page_id, action_name, task_state_id):
        if self.form_class:
            ...
            if form.is_valid():
                redirect_to = (
                    self.task.on_action(self.task_state, request.user, self.action_name, **form.cleaned_data)
                    or self.redirect_to
                )
...
class CollectWorkflowActionData(BaseWorkflowFormView):
    ...
    def post(self, request, page_id, action_name, task_state_id):
        ...
        if form.is_valid():
            return render_modal_workflow(
                request, "", None, {}, json_data={"step": "success", "cleaned_data": form.cleaned_data}
            )
        elif self.action_modal and request.headers.get("x-requested-with") == "XMLHttpRequest":
            # show form errors
            return self.render_modal_form(request, form)
...
def preview_revision_for_task(request, page_id, task_id):
    ...
    try:
        ...
        messages.error(
            request,
            _("The page '{0}' is not currently awaiting moderation in task '{1}'.").format(
                page.get_admin_display_title(), task.name
            ),
        )
    ...
    return page_to_view.make_preview_request(
        request, page.default_preview_mode, extra_request_attrs={"revision_id": revision.id}
    )
