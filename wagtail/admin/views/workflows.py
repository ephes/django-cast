            ...
            if pages_formset.is_valid():
                ...
                if success_message is not None:
                    messages.success(
                        self.request,
                        success_message,
                        buttons=[messages.button(reverse(self.edit_url_name, args=(self.object.id,)), _("Edit"))],
                    )
...
class CreateTask(CreateView):
    ...
    def get_add_url(self):
        return reverse(
            self.add_url_name,
            kwargs={"app_label": self.kwargs.get("app_label"), "model_name": self.kwargs.get("model_name")},
        )
...
def task_chooser(request):
    ...
    task_type_choices = [(model, model.get_verbose_name()) for model in task_models]
    ...
    if "q" in request.GET or "p" in request.GET or "task_type" in request.GET:
        ...
        return TemplateResponse(
            request,
            "wagtailadmin/workflows/task_chooser/includes/results.html",
            {
                "task_types": task_types,
            ...
            if request.method == "POST":
                ...
                if createform.is_valid():
                    ...
                    response = render_modal_workflow(
                        ...
                        None,
                        None,
                        json_data={"step": "task_chosen", "result": get_task_result_data(task)},
                    )
        ...
        return render_modal_workflow(
            ...
            None,
            {
                ...
                "add_url": reverse("wagtailadmin_workflows:task_chooser") + "?" + request.GET.urlencode()
                if create_model
                else None,
            },
...
def task_chosen(request, task_id):
    ...
    return render_modal_workflow(
        request, None, None, None, json_data={"step": "task_chosen", "result": get_task_result_data(task)}
    )
