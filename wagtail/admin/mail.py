    ...
    with OpenedConnection(connection) as open_connection:
        ...
        for recipient in email_recipients:
            try:
                ...
                logger.exception("Failed to send notification email '%s' to %s", email_subject, recipient.email)
...
class EmailNotificationMixin:
    ...
    def get_valid_recipients(self, instance, **kwargs):
        ...
        return {
            ...
            if recipient.is_active
            and recipient.email
            and getattr(UserProfile.get_for_user(recipient), self.notification + "_notifications")
        }
            ...
            with OpenedConnection(connection) as open_connection:
                ...
                for recipient in recipients:
                    try:
                        ...
                        # Send email
                        send_mail(
                            email_subject, email_content, [recipient.email], connection=open_connection, **kwargs
                        )
                        sent_count += 1
                    except Exception:
                        logger.exception(
                            "Failed to send notification email '%s' to %s", email_subject, recipient.email
                        )
