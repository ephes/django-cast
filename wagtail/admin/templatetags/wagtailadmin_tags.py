...
def main_nav_js():
    if slim_sidebar_enabled():
        ...
        return Media(js=[versioned_static("wagtailadmin/js/sidebar-legacy.js")]) + admin_menu.media["js"]
...
def render_with_errors(bound_field):
    ...
    if bound_field.errors and hasattr(widget, "render_with_errors"):
        return widget.render_with_errors(
            bound_field.html_name, bound_field.value(), attrs={"id": bound_field.auto_id}, errors=bound_field.errors
        )
    else:
...
def timesince_last_update(last_update, time_prefix="", use_shorthand=True):
    ...
    if last_update.date() == datetime.today().date():
        ...
        return time_str if not time_prefix else f"{time_prefix} {time_str}"
    else:
        if use_shorthand:
