...
class WorkflowPageForm(forms.ModelForm):
    page = forms.ModelChoiceField(
        queryset=Page.objects.all(), widget=widgets.AdminPageChooser(target_models=[Page], can_choose_root=True)
    )
        ...
        try:
            ...
            if not self.errors and existing_workflow != self.cleaned_data["workflow"]:
                ...
                self.add_error(
                    "page",
                    ValidationError(
                        _("This page already has workflow '{0}' assigned.").format(existing_workflow),
...
def get_task_form_class(task_model, for_edit=False):
    ...
    form_class = forms.modelform_factory(
        task_model, form=BaseTaskForm, fields=fields, widgets=getattr(task_model, "admin_form_widgets", {})
    )
