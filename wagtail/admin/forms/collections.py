...
class BaseGroupCollectionMemberPermissionFormSet(forms.BaseFormSet):
    ...
    def __init__(self, data=None, files=None, instance=None, prefix=None):
        ...
        for collection, collection_permissions in groupby(
            ...
            initial_data.append(
                {"collection": collection, "permissions": [cp.permission for cp in collection_permissions]}
            )
...
def collection_member_permission_formset_factory(model, permission_types, template, default_prefix=None):
    ...
    class CollectionMemberPermissionsForm(forms.Form):
        ...
        permissions = PermissionMultipleChoiceField(
            queryset=permission_queryset, required=False, widget=forms.CheckboxSelectMultiple
        )
    ...
    return forms.formset_factory(
        CollectionMemberPermissionsForm, formset=GroupCollectionMemberPermissionFormSet, extra=0, can_delete=True
    )
