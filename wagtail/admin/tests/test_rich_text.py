...
class TestDefaultRichText(BaseRichTextEditHandlerTestCase, WagtailTestUtils):
    ...
    def test_default_editor_in_rich_text_field(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "defaultrichtextfieldpage", self.root_page.id))
        )
    ...
    def test_default_editor_in_rich_text_block(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "defaultrichblockfieldpage", self.root_page.id))
        )
...
class TestCustomDefaultRichText(BaseRichTextEditHandlerTestCase, WagtailTestUtils):
    ...
    def test_custom_editor_in_rich_text_field(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "customrichtextfieldpage", self.root_page.id))
        )
    ...
    def test_custom_editor_in_rich_text_block(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "customrichblockfieldpage", self.root_page.id))
        )
...
class TestHalloJsWithCustomPluginOptions(BaseRichTextEditHandlerTestCase, WagtailTestUtils):
    ...
    def test_custom_editor_in_rich_text_field(self):
        ...
        self.assertContains(
            response, 'makeHalloRichTextEditable("id_body", {"halloheadings": {"formatBlocks": ["p", "h2"]}});'
        )
    ...
    def test_custom_editor_in_rich_text_block(self):
        ...
        # Check that the custom plugin options are being passed in the hallo initialiser
        self.assertIn(
            'makeHalloRichTextEditable("body", {"halloheadings": {"formatBlocks": ["p", "h2"]}});', form_html
        )
...
class TestHalloJsWithFeaturesKwarg(BaseRichTextEditHandlerTestCase, WagtailTestUtils):
    ...
    def test_features_list_on_rich_text_field(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "richtextfieldwithfeaturespage", self.root_page.id))
        )
...
class TestDraftailWithAdditionalFeatures(BaseRichTextEditHandlerTestCase, WagtailTestUtils):
    ...
    @override_settings(
        WAGTAILADMIN_RICH_TEXT_EDITORS={
            "default": {
                "WIDGET": "wagtail.admin.rich_text.DraftailRichTextArea",
                "OPTIONS": {"features": ["h2", "code", "blockquote", "strikethrough", "subscript", "superscript"]},
            },
        }
    )
...
class TestWidgetWhitelisting(TestCase, WagtailTestUtils):
    def test_default_whitelist(self):
        ...
        result = widget.value_from_datadict(
            {"body": "<h2>heading</h2><script>script</script><blockquote>blockquote</blockquote>"}, {}, "body"
        )
    ...
    def test_embed_conversion_with_custom_whitelist(self):
        ...
        self.assertHTMLEqual(
            result, '<p>image <embed embedtype="image" id="123" format="left" alt="test alt" /> embed </p>'
        )
