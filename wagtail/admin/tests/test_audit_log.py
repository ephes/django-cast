...
class TestAuditLogAdmin(TestCase, WagtailTestUtils):
    ...
    def test_page_history(self):
        ...
        self.assertContains(response, "Added the &#x27;Private, accessible to logged-in users&#x27; view restriction")
        self.assertContains(
            response, "Updated the view restriction to &#x27;Private, accessible with the following password&#x27;"
        )
        self.assertContains(
            response, "Removed the &#x27;Private, accessible with the following password&#x27; view restriction"
        )
    ...
    def test_create_and_publish_does_not_log_revision_save(self):
        ...
        response = self.client.post(
            reverse("wagtailadmin_pages:add", args=("tests", "simplepage", self.root_page.id)), post_data, follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_revert_and_publish_logs_reversion_and_publish(self):
        ...
        self.assertListEqual(list(entries), ["wagtail.publish", "wagtail.rename", "wagtail.revert", "wagtail.create"])
