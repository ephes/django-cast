...
class TestTextFieldComparison(TestFieldComparison):
    ...
    def test_from_none_to_value_only_shows_addition(self):
        comparison = self.comparison_class(
            SimplePage._meta.get_field("content"), SimplePage(content=None), SimplePage(content="Added content")
        )
    ...
    def test_from_value_to_none_only_shows_deletion(self):
        comparison = self.comparison_class(
            SimplePage._meta.get_field("content"), SimplePage(content="Removed content"), SimplePage(content=None)
        )
...
class TestChoiceFieldComparison(TestCase):
    ...
    def test_has_changed(self):
        ...
        self.assertEqual(
            comparison.htmldiff(), '<span class="deletion">Public</span><span class="addition">Private</span>'
        )
        self.assertIsInstance(comparison.htmldiff(), SafeString)
        self.assertTrue(comparison.has_changed())
...
class TestForeignObjectComparisonWithCustomPK(TestCase):
    ...
    def test_has_changed(self):
        ...
        self.assertEqual(
            comparison.htmldiff(), '<span class="deletion">Advert 1</span><span class="addition">Advert 2</span>'
        )
        self.assertIsInstance(comparison.htmldiff(), SafeString)
        self.assertTrue(comparison.has_changed())
