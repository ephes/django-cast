...
class TestPageCreation(TestCase, WagtailTestUtils):
    ...
    def test_add_subpage_with_one_valid_subpage_type(self):
        ...
        self.assertRedirects(
            response, reverse("wagtailadmin_pages:add", args=("tests", "businesschild", business_subindex.id))
        )
    ...
    def test_add_subpage_with_next_param(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:add_subpage", args=(self.root_page.id,)), {"next": "/admin/users/"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_page_with_non_model_field(self):
        ...
        """
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "formclassadditionalfieldpage", self.root_page.id))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailadmin/pages/create.html")
    ...
    def test_cannot_create_page_when_can_create_at_returns_false(self):
        ...
        add_url = reverse(
            "wagtailadmin_pages:add",
            args=[SingletonPage._meta.app_label, SingletonPage._meta.model_name, self.root_page.pk],
        )
...
class TestPerRequestEditHandler(TestCase, WagtailTestUtils):
    ...
    def setUp(self):
        ...
        GroupPagePermission.objects.create(
            group=Group.objects.get(name="Site-wide editors"), page=self.root_page, permission_type="add"
        )
...
class TestInlinePanelMedia(TestCase, WagtailTestUtils):
    ...
    def test_inline_panel_media(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_pages:add", args=("tests", "sectionedrichtextpage", homepage.id))
        )
        self.assertEqual(response.status_code, 200)
...
class TestLocaleSelector(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector(self):
        ...
        add_translation_url = reverse(
            "wagtailadmin_pages:add", args=["tests", "eventpage", self.translated_events_page.id]
        )
    ...
    def test_locale_selector_not_present_when_i18n_disabled(self):
        ...
        self.assertNotContains(
            response, f'<a href="{add_translation_url}" aria-label="French" class="u-link is-live">'
        )
...
class TestLocaleSelectorOnRootPage(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector(self):
        ...
        add_translation_url = (
            reverse("wagtailadmin_pages:add", args=["demosite", "homepage", self.root_page.id]) + "?locale=fr"
        )
        self.assertContains(response, f'<a href="{add_translation_url}" aria-label="French" class="u-link is-live">')
