...
class TestPageEdit(TestCase, WagtailTestUtils):
    ...
    def test_page_edit(self):
        ...
        self.assertContains(
            response, '<button type="submit" name="action-panic" value="Panic!" class="button">Panic!</button>'
        )
        self.assertContains(response, "testapp/js/siren.js")

        # test construct_page_action_menu hook
        self.assertContains(
            response, '<button type="submit" name="action-relax" value="Relax." class="button">Relax.</button>'
        )
    ...
    def test_edit_post_scheduled(self):
        ...
        self.assertTrue(
            PageRevision.objects.filter(page=child_page_new, content_json__contains=str(go_live_at.date())).exists()
        )
        self.assertTrue(
    ...
    def test_edit_post_publish_now_an_already_scheduled_published_page(self):
        ...
        self.assertEqual(child_page_new.title, post_data["title"], "A published page should have the new title")
    ...
    def test_page_edit_post_submit(self):
        ...
        self.assertEqual(
            child_page_new.current_workflow_state.status, child_page_new.current_workflow_state.STATUS_IN_PROGRESS
        )
    ...
    def test_preview_on_edit_no_session_key(self):
        ...
        self.assertContains(response, "<title>Wagtail - Preview error</title>", html=True)
    ...
    def test_editor_page_shows_live_url_in_status_when_draft_edits_exist(self):
        ...
        input_field_for_live_slug = (
            '<input type="text" name="slug" value="hello-world" id="id_slug" maxlength="255" required />'
        )
    ...
    def test_editor_page_shows_custom_live_url_in_status_when_draft_edits_exist(self):
        ...
        input_field_for_draft_slug = '<input type="text" name="slug" value="revised-slug-in-draft-only" id="id_slug" maxlength="255" required />'
        input_field_for_live_slug = (
            '<input type="text" name="slug" value="mars-landing" id="id_slug" maxlength="255" required />'
        )
...
class TestChildRelationsOnSuperclass(TestCase, WagtailTestUtils):
    ...
    def test_get_edit_form(self):
        ...
        self.assertContains(response, '<option value="1" selected="selected">test_advert</option>', html=True)
...
class TestParentalM2M(TestCase, WagtailTestUtils):
    ...
    def test_create_and_save(self):
        ...
        response = self.client.post(
            reverse("wagtailadmin_pages:add", args=("tests", "eventpage", self.events_index.id)), post_data
        )
...
class TestLocaleSelector(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector_not_present_when_i18n_disabled(self):
        ...
        self.assertNotContains(
            response, f'<a href="{edit_translation_url}" aria-label="French" class="u-link is-live">'
        )
...
class TestPageSubscriptionSettings(TestCase, WagtailTestUtils):
    ...
    def test_commment_notifications_switched_off(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, '<input type="checkbox" name="comment_notifications" id="id_comment_notifications">'
        )
    ...
    def test_commment_notifications_switched_on(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, '<input type="checkbox" name="comment_notifications" id="id_comment_notifications" checked>'
        )
...
class TestCommenting(TestCase, WagtailTestUtils):
    ...
    def test_new_comment(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, 'test@email.com has updated comments on "I\'ve been edited! (simple page)"'
        )
    ...
    def test_resolve_comment(self):
        ...
        self.assertEqual(
            mail.outbox[1].subject, 'test@email.com has updated comments on "I\'ve been edited! (simple page)"'
        )
        self.assertIn('Resolved comments:\n - "A test comment"\n\n', mail.outbox[1].body)
    ...
    def test_new_reply(self):
        ...
        self.assertEqual(
            mail.outbox[2].subject, 'test@email.com has updated comments on "I\'ve been edited! (simple page)"'
        )
