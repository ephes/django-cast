...
class TestPageExplorer(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.old_page = StandardIndex(
            title="Old page", slug="old-page", latest_revision_created_at=local_datetime(2010, 1, 1)
        )
        self.root_page.add_child(instance=self.old_page)

        self.new_page = SimplePage(
            title="New page", slug="new-page", content="hello", latest_revision_created_at=local_datetime(2016, 1, 1)
        )
    ...
    def test_explore_root_shows_icon(self):
        ...
        self.assertContains(
            response, ("""<a href="/admin/sites/" class="icon icon-site" """ """title="Sites menu"></a>""")
        )
    ...
    def test_ordering_by_last_revision_forward(self):
        response = self.client.get(
            reverse("wagtailadmin_explore", args=(self.root_page.id,)), {"ordering": "latest_revision_created_at"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailadmin/pages/index.html")
    ...
    def test_invalid_ordering(self):
        response = self.client.get(
            reverse("wagtailadmin_explore", args=(self.root_page.id,)), {"ordering": "invalid_order"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailadmin/pages/index.html")
        self.assertEqual(response.context["ordering"], "-latest_revision_created_at")
    ...
    def test_construct_explorer_page_queryset_hook(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_explore", args=(self.root_page.id,)), {"polite_pages_only": "yes_please"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailadmin/pages/index.html")
        page_ids = [page.id for page in response.context["pages"]]
...
class TestPageExplorerSignposting(TestCase, WagtailTestUtils):
    ...
    def test_nonadmin_at_root(self):
        ...
        GroupPagePermission.objects.create(
            group=Group.objects.get(name="Site-wide editors"), page=self.no_site_page, permission_type="add"
        )
