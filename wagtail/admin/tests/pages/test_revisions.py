...
class TestRevisions(TestCase, WagtailTestUtils):
    ...
    def test_get_revisions_index(self):
        ...
        last_christmas_preview_url = reverse(
            "wagtailadmin_pages:revisions_view", args=(self.christmas_event.id, self.last_christmas_revision.id)
        )
        last_christmas_revert_url = reverse(
            "wagtailadmin_pages:revisions_revert", args=(self.christmas_event.id, self.last_christmas_revision.id)
        )
        ...
        this_christmas_preview_url = reverse(
            "wagtailadmin_pages:revisions_view", args=(self.christmas_event.id, self.this_christmas_revision.id)
        )
        this_christmas_revert_url = reverse(
            "wagtailadmin_pages:revisions_revert", args=(self.christmas_event.id, self.this_christmas_revision.id)
        )
    ...
    def test_scheduled_revision(self):
        ...
        this_christmas_unschedule_url = reverse(
            "wagtailadmin_pages:revisions_unschedule", args=(self.christmas_event.id, self.this_christmas_revision.id)
        )
        response = self.client.get(reverse("wagtailadmin_pages:revisions_index", args=(self.christmas_event.id,)))
        self.assertEqual(response.status_code, 200)
...
class TestCompareRevisionsWithNonModelField(TestCase, WagtailTestUtils):
    ...
    def setUp(self):
        ...
        self.test_page = FormClassAdditionalFieldPage(
            title="A Statement", slug="a-statement", location="Early Morning Cafe, Mainland, NZ", body="<p>hello</p>"
        )
    ...
    def test_base_form_class_used(self):
        ...
        self.assertContains(
            response, '<input type="text" name="code" required id="id_code" maxlength="5" />', html=True
        )
...
class TestRevisionsUnschedule(TestCase, WagtailTestUtils):
    ...
    def test_unschedule_view_invalid_revision_id(self):
        ...
        # Get unschedule page
        response = self.client.get(
            reverse("wagtailadmin_pages:revisions_unschedule", args=(self.christmas_event.id, 67894))
        )
