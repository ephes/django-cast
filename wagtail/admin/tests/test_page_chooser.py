...
class TestAutomaticRootPageDetection(TestCase, WagtailTestUtils):
    ...
    def make_event_section(self, name):
        ...
        event_index.add_child(
            instance=EventPage(
                title="First Event", location="Bar", audience="public", cost="free", date_from="2001-01-01"
            )
        )
        event_index.add_child(
            instance=EventPage(
                title="Second Event", location="Baz", audience="public", cost="free", date_from="2001-01-01"
            )
        )
        return event_index
...
class TestChooserExternalLink(TestCase, WagtailTestUtils):
    ...
    def test_create_link(self):
        response = self.post(
            {"external-link-chooser-url": "http://www.example.com/", "external-link-chooser-link_text": "example"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_invalid_url(self):
        response = self.post(
            {"external-link-chooser-url": "ntp://www.example.com", "external-link-chooser-link_text": "example"}
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content.decode())
    ...
    def test_convert_external_to_internal_link(self):
        response = self.post(
            {"external-link-chooser-url": "http://localhost/about/", "external-link-chooser-link_text": "about"}
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content.decode())
        self.assertEqual(response_json["step"], "external_link_chosen")
    ...
    def test_convert_external_link_with_query_parameters_to_internal_link(self):
        response = self.post(
            {"external-link-chooser-url": "http://localhost/about?test=1", "external-link-chooser-link_text": "about"}
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content.decode())
...
class TestChooserAnchorLink(TestCase, WagtailTestUtils):
    ...
    def test_create_link(self):
        response = self.post(
            {"anchor-link-chooser-url": "exampleanchor", "anchor-link-chooser-link_text": "Example Anchor Text"}
        )
        result = json.loads(response.content.decode())["result"]
...
class TestChooserEmailLink(TestCase, WagtailTestUtils):
    ...
    def test_create_link(self):
        response = self.post(
            {"email-link-chooser-email_address": "example@example.com", "email-link-chooser-link_text": "contact"}
        )
        result = json.loads(response.content.decode())["result"]
