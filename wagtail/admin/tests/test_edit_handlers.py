...
class TestGetFormForModel(TestCase):
    def test_get_form_without_model(self):
        ...
        with self.assertRaisesMessage(
            AttributeError,
            "ObjectList is not bound to a model yet. " "Use `.bind_to(model=model)` before using this method.",
        ):
    ...
    def test_get_form_for_model_with_specific_fields(self):
        EventPageForm = get_form_for_model(
            EventPage, form_class=WagtailAdminPageForm, fields=["date_from"], formsets=["speakers"]
        )
        form = EventPageForm()
    ...
    def test_get_form_for_model_with_excluded_fields(self):
        EventPageForm = get_form_for_model(
            EventPage, form_class=WagtailAdminPageForm, exclude=["title"], exclude_formsets=["related_links"]
        )
        form = EventPageForm()
    ...
    def test_get_form_for_model_with_widget_overides_by_class(self):
        EventPageForm = get_form_for_model(
            EventPage, form_class=WagtailAdminPageForm, widgets={"date_from": forms.PasswordInput}
        )
        form = EventPageForm()
    ...
    def test_tag_widget_is_passed_tag_model(self):
        RestaurantPageForm = get_form_for_model(RestaurantPage, form_class=WagtailAdminPageForm)
...
class TestExtractPanelDefinitionsFromModelClass(TestCase):
    ...
    def test_can_build_panel_list(self):
        ...
        self.assertTrue(
            any([isinstance(panel, RichTextFieldPanel) and panel.field_name == "body" for panel in panels])
        )
...
class TestFieldPanel(TestCase):
    def setUp(self):
        ...
        self.EventPageForm = get_form_for_model(EventPage, form_class=WagtailAdminPageForm, formsets=[])
        self.event = EventPage(
            title="Abergavenny sheepdog trials", date_from=date(2014, 7, 20), date_to=date(2014, 7, 21)
        )
...
class TestFieldRowPanelWithChooser(TestCase):
    def setUp(self):
        ...
        self.EventPageForm = get_form_for_model(EventPage, form_class=WagtailAdminPageForm, formsets=[])
        self.event = EventPage(
            title="Abergavenny sheepdog trials", date_from=date(2014, 7, 19), date_to=date(2014, 7, 21)
        )
...
class TestPageChooserPanel(TestCase):
    ...
    def test_render_js_init_with_can_choose_root_true(self):
        ...
        my_page_object_list = ObjectList([PageChooserPanel("page", can_choose_root=True)]).bind_to(
    ...
    def test_override_page_type(self):
        ...
        my_page_object_list = ObjectList([PageChooserPanel("page", "tests.EventPage")]).bind_to(
    ...
    def test_autodetect_page_type(self):
        ...
        PageChooserForm = my_page_object_list.get_form_class()
        form = PageChooserForm(instance=self.test_instance)
        page_chooser_panel = my_page_chooser_panel.bind_to(instance=self.test_instance, form=form)
    ...
    def test_target_models(self):
        result = PageChooserPanel("page", "wagtailcore.site").bind_to(model=PageChooserModel).target_models()
    ...
    def test_target_models_nonexistent_type(self):
        result = PageChooserPanel("page", "snowman.lorry").bind_to(model=PageChooserModel)
        self.assertRaises(ImproperlyConfigured, result.target_models)
...
class TestInlinePanel(TestCase, WagtailTestUtils):
    ...
    def test_render(self):
        ...
        self.assertTagInHTML(
            '<input id="id_speakers-0-id" name="speakers-0-id" type="hidden">', result, allow_extra_attrs=True
        )
        self.assertTagInHTML(
            '<input id="id_speakers-0-DELETE" name="speakers-0-DELETE" type="hidden">', result, allow_extra_attrs=True
        )
        self.assertTagInHTML(
            '<input id="id_speakers-0-ORDER" name="speakers-0-ORDER" type="hidden">', result, allow_extra_attrs=True
        )
...
class TestCommentPanel(TestCase, WagtailTestUtils):
    ...
    def setUp(self):
        ...
        self.object_list = ObjectList([CommentPanel()]).bind_to(model=EventPage, request=self.request)
        ...
        self.comment = Comment.objects.create(
            page=self.event_page, text="test", user=self.other_user, contentpath="test_contentpath"
        )
