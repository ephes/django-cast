...
class TestAdminPageListing(AdminAPITestCase, TestPageListing):
    ...
    def test_all_fields_then_remove_something(self):
        response = self.get_response(
            type="demosite.BlogEntryPage", fields="*,-title,-admin_display_title,-date,-seo_title,-status"
        )
        content = json.loads(response.content.decode("UTF-8"))
    ...
    def test_fields_foreign_key(self):
        ...
        for page in content["items"]:
            ...
            if feed_image is not None:
                ...
                self.assertEqual(
                    feed_image["meta"]["detail_url"], "http://localhost/admin/api/main/images/%d/" % feed_image["id"]
                )
    ...
    def test_fields_descendants(self):
        ...
        for page in content["items"]:
            ...
            self.assertEqual(
                descendants["listing_url"], "http://localhost/admin/api/main/pages/?descendant_of=%d" % page["id"]
            )
...
class TestAdminPageDetail(AdminAPITestCase, TestPageDetail):
    ...
    def test_basic(self):
        ...
        self.assertEqual(content["meta"]["status"], {"status": "live", "live": True, "has_unpublished_changes": False})
        ...
        self.assertEqual(
            ...
            {"count": 0, "listing_url": "http://localhost/admin/api/main/pages/?child_of=16"},
        )
        ...
        self.assertEqual(
            set(content["__types"]["demosite.BlogIndexPage"].keys()), {"verbose_name", "verbose_name_plural"}
        )
    ...
    def test_meta_status_draft(self):
        ...
        self.assertIn("status", content["meta"])
        self.assertEqual(
            content["meta"]["status"], {"status": "draft", "live": False, "has_unpublished_changes": True}
        )
    ...
    def test_meta_status_live_draft(self):
        ...
        self.assertIn("status", content["meta"])
        self.assertEqual(
            content["meta"]["status"], {"status": "live + draft", "live": True, "has_unpublished_changes": True}
        )
    ...
    def test_meta_status_scheduled(self):
        ...
        self.assertIn("status", content["meta"])
        self.assertEqual(
            content["meta"]["status"], {"status": "scheduled", "live": False, "has_unpublished_changes": True}
        )
    ...
    def test_meta_status_expired(self):
        ...
        self.assertIn("status", content["meta"])
        self.assertEqual(
            content["meta"]["status"], {"status": "expired", "live": False, "has_unpublished_changes": True}
        )
    ...
    def test_meta_children_for_parent(self):
        ...
        self.assertEqual(
            content["meta"]["children"],
            {"count": 5, "listing_url": "http://localhost/admin/api/main/pages/?child_of=2"},
        )
    ...
    def test_meta_descendants(self):
        ...
        self.assertEqual(
            ...
            {"count": 18, "listing_url": "http://localhost/admin/api/main/pages/?descendant_of=2"},
        )
    ...
    def test_remove_all_meta_fields(self):
        response = self.get_response(
            16,
            fields="-type,-detail_url,-slug,-first_published_at,-html_url,-descendants,-latest_revision_created_at,-children,-ancestors,-show_in_menus,-seo_title,-parent,-status,-search_description",
        )
        content = json.loads(response.content.decode("UTF-8"))
