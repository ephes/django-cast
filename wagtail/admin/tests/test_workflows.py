...
class TestWorkflowsCreateView(TestCase, WagtailTestUtils):
    ...
    def test_page_already_has_workflow_check(self):
        ...
        self.assertFormsetError(
            response, "pages_formset", 0, "page", ["This page already has workflow 'existing_workflow' assigned."]
        )
...
class TestWorkflowsEditView(TestCase, WagtailTestUtils):
    ...
    def test_duplicate_page_check(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormsetError(
            response, "pages_formset", None, None, ["You cannot assign this workflow to the same page multiple times."]
        )
...
class TestRemoveWorkflow(TestCase, WagtailTestUtils):
    ...
    def post(self, post_data={}):
        return self.client.post(
            reverse("wagtailadmin_workflows:remove", args=[self.page.id, self.workflow.id]), post_data
        )
    ...
    def post(self, post_data={}):
        return self.client.post(
            reverse(
                ...
                kwargs={"app_label": SimpleTask._meta.app_label, "model_name": SimpleTask._meta.model_name},
            ),
            post_data,
        )
...
class TestSubmitToWorkflow(TestCase, WagtailTestUtils):
    ...
    def test_submit_for_approval_changes_status_in_header_meta(self):
        ...
        self.assertRegex(response.content.decode("utf-8"), fr"Awaiting[\s|\n]+{self.page.current_workflow_task.name}")
...
class TestApproveRejectWorkflow(TestCase, WagtailTestUtils):
    ...
    def test_approve_task_and_workflow(self):
        ...
        self.client.post(
            reverse(
                ...
                args=(self.page.id, "approve", self.page.current_workflow_task_state.id),
            ),
            {"comment": "my comment"},
        )
        ...
        self.assertFalse(
            page.has_unpublished_changes, "Approving moderation failed to set has_unpublished_changes=False"
        )
    ...
    def test_workflow_action_view_bad_page_id(self):
        ...
        response = self.client.post(
            reverse(
                "wagtailadmin_pages:workflow_action",
                args=(127777777777, "approve", self.page.current_workflow_task_state.id),
            )
        )
    ...
    def test_edit_view_workflow_cancellation_not_in_group(self):
        ...
        # Post
        response = self.client.post(
            reverse("wagtailadmin_pages:edit", args=(self.page.id,)), {"action-cancel-workflow": "True"}
        )
...
class TestDisableViews(TestCase, WagtailTestUtils):
    ...
    def test_disable_workflow(self):
        ...
        self.assertEqual(
            TaskState.objects.filter(
                workflow_state__workflow=self.workflow, status=TaskState.STATUS_IN_PROGRESS
            ).count(),
            0,
        )
    ...
    def test_disable_task_view(self):
        ...
        self.assertEqual(
            ...
            "This task is in progress on 1 page. Disabling this task will cause it to be skipped in the moderation workflow.",
        )
        ...
        self.assertEqual(
            response.context["warning_message"],
            "This task is in progress on 0 pages. Disabling this task will cause it to be skipped in the moderation workflow.",
        )
...
class TestTaskChooserView(TestCase, WagtailTestUtils):
    ...
    def test_get_with_create_model_selected(self):
        response = self.client.get(
            reverse("wagtailadmin_workflows:task_chooser") + "?create_model=wagtailcore.GroupApprovalTask"
        )
    ...
    def test_post_with_non_task_create_model_selected(self):
        response = self.client.post(
            reverse("wagtailadmin_workflows:task_chooser") + "?create_model=wagtailcore.Page", self.get_post_data()
        )
    ...
    def test_post_with_base_task_create_model_selected(self):
        # Task is technically a subclass of itself so we need an extra test for it
        response = self.client.post(
            reverse("wagtailadmin_workflows:task_chooser") + "?create_model=wagtailcore.Task", self.get_post_data()
        )
...
class TestWorkflowStatus(TestCase, WagtailTestUtils):
    ...
    def test_workflow_status_modal(self):
        ...
        self.assertIn(f"{self.task_1.name}: In progress", html)
        self.assertIn(self.task_2.name, html)
        self.assertIn(f"{self.task_2.name}: Not started", html)
