...
class TestHome(TestCase, WagtailTestUtils):
    ...
    def test_admin_menu(self):
        ...
        if DJANGO_VERSION >= (3, 1):
            self.assertContains(response, '<script src="/static/testapp/js/kittens.js"></script>', html=True)
        else:
            self.assertContains(
                response, '<script type="text/javascript" src="/static/testapp/js/kittens.js"></script>', html=True
            )
    ...
    def test_dashboard_panels(self):
        ...
        if DJANGO_VERSION >= (3, 1):
            ...
            self.assertContains(
                response, '<script type="text/javascript" src="/static/testapp/js/clippy.js"></script>', html=True
            )
...
class TestSendMail(TestCase):
    ...
    def test_send_html_email(self):
        ...
        send_mail(
            "Test HTML subject", "TEXT content", ["has.html@email.com"], html_message="<h2>Test HTML content</h2>"
        )
...
class TestTagsAutocomplete(TestCase, WagtailTestUtils):
    ...
    def test_tags_autocomplete_custom_model(self):
        response = self.client.get(
            reverse("wagtailadmin_tag_model_autocomplete", args=("tests", "restauranttag")), {"term": "ital"}
        )
        ...
        response = self.client.get(
            reverse("wagtailadmin_tag_model_autocomplete", args=("tests", "restauranttag")), {"term": "test"}
        )
