...
class HalloFormatPlugin(HalloPlugin):
    ...
    def construct_plugins_list(self, plugins):
        plugins.setdefault(
            self.name, {"formattings": {"bold": False, "italic": False, "strikeThrough": False, "underline": False}}
        )
...
class HalloRichTextArea(widgets.Textarea):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.plugins = CORE_HALLO_PLUGINS + list(
            filter(None, [features.get_editor_plugin("hallo", feature_name) for feature_name in self.features])
        )
