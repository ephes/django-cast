...
class TestLocaleCreateView(TestCase, WagtailTestUtils):
    ...
    def test_duplicate_not_allowed(self):
        ...
        self.assertFormError(
            response, "form", "language_code", ["Select a valid choice. en is not one of the available choices."]
        )
    ...
    def test_language_code_must_be_in_settings(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(
            response, "form", "language_code", ["Select a valid choice. ja is not one of the available choices."]
        )
...
class TestLocaleEditView(TestCase, WagtailTestUtils):
    ...
    def test_simple(self):
        ...
        self.assertEqual(
            ...
            [("en", "English"), ("fr", "French")],  # Note: Current value is displayed even though it's in use
        )
...
class TestLocaleDeleteView(TestCase, WagtailTestUtils):
    ...
    def test_cannot_delete_last_remaining_locale(self):
        ...
        self.assertEqual(messages[0].level_tag, "error")
        self.assertEqual(
            messages[0].message, "This locale cannot be deleted because there are no other locales.\n\n\n\n\n"
        )
