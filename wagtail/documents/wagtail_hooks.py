...
def register_documents_menu_item():
    return DocumentsMenuItem(
        _("Documents"), reverse("wagtaildocs:index"), name="documents", icon_name="doc-full-inverse", order=400
    )
        ...
        if not restriction.accept_request(request):
            if restriction.restriction_type == BaseViewRestriction.PASSWORD:
                ...
                password_required_template = getattr(
                    settings, "DOCUMENT_PASSWORD_REQUIRED_TEMPLATE", "wagtaildocs/password_required.html"
                )
