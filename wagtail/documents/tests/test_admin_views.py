...
from wagtail.tests.testapp.models import CustomDocument, CustomDocumentWithAuthor, EventPage, EventPageRelatedLink
...
class TestDocumentAddView(TestCase, WagtailTestUtils):
    ...
    def test_post_with_collections(self):
        ...
        root_collection = Collection.get_first_root_node()
        self.assertEqual(models.Document.objects.get(title="Test document").collection, evil_plans_collection)
    ...
    def test_unique_together_validation_error(self):
        ...
        CustomDocument.objects.create(
            title="Test document", file=get_test_document_file(), collection=evil_plans_collection
        )
...
class TestDocumentAddViewWithLimitedCollectionPermissions(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        GroupCollectionPermission.objects.create(
            group=conspirators_group, collection=self.evil_plans_collection, permission=add_doc_permission
        )
    ...
    def test_post(self):
        ...
        self.assertTrue(models.Document.objects.filter(title="Test document").exists())
        self.assertEqual(models.Document.objects.get(title="Test document").collection, self.evil_plans_collection)
...
class TestDocumentEditViewWithCustomDocumentModel(TestCase, WagtailTestUtils):
    ...
    def test_unique_together_validation_error(self):
        ...
        # another document with a title to collide with
        CustomDocument.objects.create(title="Updated", file=get_test_document_file(), collection=evil_plans_collection)
...
class TestMultipleDocumentUploader(TestCase, WagtailTestUtils):
    ...
    def test_add_post(self):
        ...
        self.assertEqual(
            response.context["delete_action"], "/admin/documents/multiple/%d/delete/" % response.context["doc"].id
        )
...
class TestMultipleCustomDocumentUploaderWithRequiredField(TestMultipleDocumentUploader):
    ...
    def test_create_from_upload_invalid_post(self):
        ...
        # Send request
        response = self.client.post(
            reverse("wagtaildocs:create_multiple_from_uploaded_document", args=(self.uploaded_document.id,)),
            {
...
class TestEditOnlyPermissions(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        GroupCollectionPermission.objects.create(
            group=self.changers_group, collection=self.root_collection, permission=change_permission
        )
    ...
    def test_get_edit(self):
        ...
        GroupCollectionPermission.objects.create(
            group=self.changers_group, collection=self.evil_plans_collection, permission=add_permission
        )
        response = self.client.get(reverse("wagtaildocs:edit", args=(self.document.id,)))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '<label for="id_collection">')
    ...
    def test_post_edit(self):
        ...
        self.assertEqual(models.Document.objects.get(id=self.document.id).collection, self.nice_plans_collection)
        ...
        self.assertEqual(models.Document.objects.get(id=self.document.id).collection, self.evil_plans_collection)
