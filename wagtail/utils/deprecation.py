...
class MovedDefinitionHandler:
    ...
    def __getattr__(self, name):
        ...
        if new_name != name:
            warnings.warn(
                f"{name} has been moved from {self.real_module.__name__} to {new_module_name} and renamed to {new_name}",
            ...
            warnings.warn(
                f"{name} has been moved from {self.real_module.__name__} to {new_module_name}",
                category=self.warning_class,
                stacklevel=2,
            )
