...
from wagtail.core.models import PAGE_PERMISSION_TYPE_CHOICES, PAGE_PERMISSION_TYPES, GroupPagePermission, Page
...
class UsernameForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if User.USERNAME_FIELD == "username":
            ...
            field.regex = r"^[\w.@+-]+$"
            field.help_text = _("Required. 30 characters or fewer. Letters, " "digits and @/./+/-/_ only.")
...
class PagePermissionsForm(forms.Form):
    ...
    page = forms.ModelChoiceField(
        queryset=Page.objects.all(), widget=AdminPageChooser(show_edit_link=False, can_choose_root=True)
    )
    permission_types = forms.MultipleChoiceField(
        choices=PAGE_PERMISSION_TYPE_CHOICES, required=False, widget=forms.CheckboxSelectMultiple
    )
...
class BaseGroupPagePermissionFormSet(forms.BaseFormSet):
    ...
    def as_admin_panel(self):
        return render_to_string("wagtailusers/groups/includes/page_permissions_formset.html", {"formset": self})
