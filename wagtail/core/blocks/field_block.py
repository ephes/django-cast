...
class TextBlock(FieldBlock):
    def __init__(
        self, required=True, help_text=None, rows=1, max_length=None, min_length=None, validators=(), **kwargs
    ):
        self.field_options = {
            "required": required,
            "help_text": help_text,
...
class BaseChoiceBlock(FieldBlock):
    ...
    def __init__(
        self, choices=None, default=None, required=True, help_text=None, widget=None, validators=(), **kwargs
    ):
...
DECONSTRUCT_ALIASES = {cls: "wagtail.core.blocks.%s" % cls.__name__ for cls in block_classes}
