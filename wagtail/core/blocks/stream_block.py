...
class BaseStreamBlock(Block):
    ...
    def clean(self, value):
        ...
        if self.meta.min_num is not None and self.meta.min_num > len(value):
            non_block_errors.append(ValidationError(_("The minimum number of items is %d") % self.meta.min_num))
        ...
        if self.meta.max_num is not None and self.meta.max_num < len(value):
            non_block_errors.append(ValidationError(_("The maximum number of items is %d") % self.meta.max_num))
            ...
            for block_name, min_max in self.meta.block_counts.items():
                ...
                if min_num is not None and min_num > block_count:
                    non_block_errors.append(
                        ValidationError("{}: {}".format(block.label, _("The minimum number of items is %d") % min_num))
                    )
                if max_num is not None and max_num < block_count:
                    non_block_errors.append(
                        ValidationError("{}: {}".format(block.label, _("The maximum number of items is %d") % max_num))
                    )
    ...
    def to_python(self, value):
        ...
        return StreamValue(
            self, [child_data for child_data in value if child_data["type"] in self.child_blocks], is_lazy=True
        )
