...
class TestChoiceBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_adapt_choice_block_with_existing_blank_choice(self):
        block = blocks.ChoiceBlock(choices=[("tea", "Tea"), ("coffee", "Coffee"), ("", "No thanks")], required=False)

        block.set_name("test_choiceblock")
        js_args = FieldBlockAdapter().js_args(block)

        self.assertEqual(list(js_args[1].choices), [("tea", "Tea"), ("coffee", "Coffee"), ("", "No thanks")])
...
class TestMultipleChoiceBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_adapt_multiple_choice_block_with_existing_blank_choice(self):
        block = blocks.MultipleChoiceBlock(
            choices=[("tea", "Tea"), ("coffee", "Coffee"), ("", "No thanks")], required=False
        )
...
class TestStructBlock(SimpleTestCase):
    ...
    def test_value_from_datadict(self):
        ...
        struct_val = block.value_from_datadict(
            {"mylink-title": "Torchbox", "mylink-link": "http://www.torchbox.com"}, {}, "mylink"
        )
...
class TestListBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_value_omitted_from_data(self):
        ...
        self.assertFalse(
            block.value_omitted_from_data(
                {"mylist-count": "1", "mylist-0-value": "hello", "mylist-0-deleted": "", "mylist-0-order": "0"},
                {},
                "mylist",
            )
        )
    ...
    def test_min_num_validation_errors(self):
        ...
        self.assertEqual(
            ...
            {"block_errors": [None], "non_block_errors": ["The minimum number of items is 2"]},
        )
    ...
    def test_max_num_validation_errors(self):
        ...
        self.assertEqual(
            catcher.exception.params,
            {"block_errors": [None, None, None], "non_block_errors": ["The maximum number of items is 2"]},
        )
...
class TestStreamBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_initialisation_with_mixins(self):
        ...
        self.assertEqual(list(block.child_blocks.keys()), ["heading", "paragraph", "intro", "by_line"])
    ...
    def test_block_counts_min_validation_errors(self):
        ...
        with self.assertRaises(ValidationError) as catcher:
            block.clean(value)
        self.assertEqual(catcher.exception.params, {"__all__": ["Char: The minimum number of items is 1"]})
    ...
    def test_block_counts_max_validation_errors(self):
        ...
        with self.assertRaises(ValidationError) as catcher:
            block.clean(value)
        self.assertEqual(catcher.exception.params, {"__all__": ["Char: The maximum number of items is 1"]})
    ...
    def test_get_prep_value_not_lazy(self):
        stream_data = [("heading", "this is my heading", "0000"), ("paragraph", "<p>this is a paragraph</p>")]
    ...
    def check_get_prep_value_nested_streamblocks(self, stream_data, is_lazy):
        ...
        self.assertEqual(len(jsonish_value), 2)
        self.assertEqual(jsonish_value["left"], [{"type": "text", "value": "some text", "id": "0000"}])
...
class TestStructBlockWithFixtures(TestCase):
    ...
    def test_bulk_to_python(self):
        ...
        self.assertEqual(result_pages, [Page.objects.get(id=2), Page.objects.get(id=3), None, Page.objects.get(id=4)])
...
class TestPageChooserBlock(TestCase):
    ...
    def test_adapt_with_target_model_multiple_strings(self):
        block = blocks.PageChooserBlock(
            help_text="pick a page, any page", page_type=["tests.SimplePage", "tests.EventPage"]
        )
    ...
    def test_deconstruct_target_model_default(self):
        block = blocks.PageChooserBlock()
        self.assertEqual(block.deconstruct(), ("wagtail.core.blocks.PageChooserBlock", (), {}))

    def test_deconstruct_target_model_string(self):
        block = blocks.PageChooserBlock(page_type="tests.SimplePage")
        self.assertEqual(
            block.deconstruct(), ("wagtail.core.blocks.PageChooserBlock", (), {"page_type": ["tests.SimplePage"]})
        )
    ...
    def test_deconstruct_target_model_multiple_strings(self):
        ...
        self.assertEqual(
            block.deconstruct(),
            ("wagtail.core.blocks.PageChooserBlock", (), {"page_type": ["tests.SimplePage", "tests.EventPage"]}),
        )
...
class TestStaticBlock(unittest.TestCase):
    ...
    def test_adapt_with_subclass_displays_default_text_if_no_admin_text(self):
        ...
        self.assertEqual(
            js_args[1],
            {"text": "Latest posts: this block has no options.", "icon": "placeholder", "label": "Latest posts"},
        )
