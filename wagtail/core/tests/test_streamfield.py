...
class TestLazyStreamField(TestCase):
    def setUp(self):
        ...
        self.with_image = StreamModel.objects.create(
            body=json.dumps([{"type": "image", "value": self.image.pk}, {"type": "text", "value": "foo"}])
        )
        self.no_image = StreamModel.objects.create(body=json.dumps([{"type": "text", "value": "foo"}]))
    ...
    def test_lazy_load_queryset(self):
        ...
        """
        with self.assertNumQueries(1):
            instances = StreamModel.objects.filter(pk__in=[self.with_image.pk, self.no_image.pk])
...
class TestStreamValueAccess(TestCase):
    def setUp(self):
        self.json_body = StreamModel.objects.create(body=json.dumps([{"type": "text", "value": "foo"}]))
        self.nonjson_body = StreamModel.objects.create(body="<h1>hello world</h1>")
...
class TestStreamFieldCountValidation(TestCase):
    ...
    def test_block_counts_minimums(self):
        ...
        with self.assertRaises(StreamBlockValidationError) as catcher:
            instance.body.stream_block.clean(instance.body)
        self.assertEqual(catcher.exception.params, {"__all__": ["Image: The minimum number of items is 1"]})
    ...
    def test_block_counts_maximums(self):
        ...
        with self.assertRaises(StreamBlockValidationError) as catcher:
            instance.body.stream_block.clean(instance.body)
        self.assertEqual(catcher.exception.params, {"__all__": ["Image: The maximum number of items is 1"]})
