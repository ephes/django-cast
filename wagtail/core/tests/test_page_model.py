...
class TestSiteRouting(TestCase):
    ...
    def setUp(self):
        ...
        self.alternate_port_default_site = Site.objects.create(
            hostname=self.default_site.hostname, port="8765", root_page=self.default_site.root_page
        )
    ...
    def test_port_in_http_host_header_is_ignored(self):
        ...
        request = HttpRequest()
        request.path = "/"
        request.META["HTTP_HOST"] = f"{self.events_site.hostname}:{self.events_site.port}"
        request.META["SERVER_PORT"] = self.alternate_port_events_site.port
        with self.assertNumQueries(1):
            self.assertEqual(Site.find_for_request(request), self.alternate_port_events_site)
...
class TestRouting(TestCase):
    ...
    def test_urls_with_multiple_sites(self):
        ...
        self.assertEqual(
            christmas_page.get_url_parts(request=request), (events_site.id, "http://events.example.com", "/christmas/")
        )
    ...
    def test_urls_with_non_root_urlconf(self):
        ...
        self.assertEqual(
            christmas_page.get_url_parts(), (default_site.id, "http://localhost", "/site/events/christmas/")
        )
...
class TestRoutingWithI18N(TestRouting):
    ...
    @override_settings(
        WAGTAIL_CONTENT_LANGUAGES=[("en", "English"), ("en-us", "English (United States)"), ("fr", "French")]
    )
    ...
    def test_urls_with_multiple_sites(self):
        ...
        self.assertEqual(
            christmas_page.get_url_parts(), (events_site.id, "http://events.example.com", "/en/christmas/")
        )
...
class TestCopyPage(TestCase):
    ...
    def test_copy_page_copies_parental_relations(self):
        ...
        self.assertEqual(
            new_christmas_event.categories.count(), 2, "Child objects (parental many to many) weren't copied"
        )
    ...
    def test_copy_page_does_not_copy_comments(self):
        ...
        self.assertFalse(new_christmas_event.comments.exists(), "Comments were copied")
    ...
    def test_copy_multi_table_inheritance(self):
        ...
        self.assertNotEqual(
            saint_patrick_event.eventpage_ptr.page_ptr.id, new_saint_patrick_event.eventpage_ptr.page_ptr.id
        )
    ...
    def test_copy_page_with_additional_excluded_fields(self):
        ...
        page = homepage.add_child(
            instance=PageWithExcludedCopyField(
                title="Discovery", slug="disco", content="NCC-1031", special_field="Context is for Kings"
            )
        )
...
class TestIssue1216(TestCase):
    ...
    def test_url_path_can_exceed_255_characters(self):
        ...
        new_christmas_event = EventPage.objects.get(id=christmas_event.id)
        expected_url_path = f"/home/{new_event_index_slug}/{new_christmas_slug}/"
