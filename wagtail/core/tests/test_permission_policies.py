            ...
            for (action, expected_result) in expected_results:
                if expected_result:
                    self.assertTrue(
                        ...
                        f"User {user} should be able to {action}, but can't",
                    )
                else:
                    self.assertFalse(
                        self.policy.user_has_permission(user, action),
                        f"User {user} should not be able to {action}, but can",
                    )
            ...
            for (action, expected_result) in expected_results:
                if expected_result:
                    self.assertTrue(
                        ...
                        f"User {user} should be able to {action} instance {instance}, but can't",
                    )
                else:
                    self.assertFalse(
                        self.policy.user_has_permission_for_instance(user, action, instance),
                        f"User {user} should not be able to {action} instance {instance}, but can",
                    )
...
class PermissionPolicyTestCase(PermissionPolicyTestUtils, TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.adder_image = Image.objects.create(
            title="imageadder's image", file=get_test_image_file(), uploaded_by_user=self.image_adder
        )

        # an image owned by 'uselessuser'
        self.useless_image = Image.objects.create(
            title="uselessuser's image", file=get_test_image_file(), uploaded_by_user=self.useless_user
        )
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        for user in self.all_users:
            self.assertResultSetEqual(
                self.policy.instances_user_has_any_permission_for(user, ["change", "delete"]), all_images
            )
...
class TestAuthenticationOnlyPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_instances_user_has_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.inactive_superuser, "change"), no_images
        )

        # the set of images editable by ordinary user includes all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.useless_user, "change"), all_images
        )

        # the set of images editable by anonymous user includes no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.anonymous_user, "change"), no_images
        )
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        # the set of images editable by superuser includes all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.superuser, ["change", "delete"]), all_images
        )

        # the set of images editable by inactive superuser includes no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.inactive_superuser, ["change", "delete"]), no_images
        )

        # the set of images editable by ordinary user includes all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.useless_user, ["change", "delete"]), all_images
        )

        # the set of images editable by anonymous user includes no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.anonymous_user, ["change", "delete"]), no_images
        )
...
class TestModelPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_instances_user_has_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.image_changer, "change"), all_images
        )

        # given the relevant model permission at the user level, a user can edit all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.oneoff_image_changer, "change"), all_images
        )

        # a user with no permission can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.useless_user, "change"), no_images
        )

        # an inactive user with the relevant permission can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.inactive_image_changer, "change"), no_images
        )

        # a user with permission, but not the matching one, can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.image_changer, "delete"), no_images
        )
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        # given the relevant model permission at the group level, a user can edit all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.image_changer, ["change", "delete"]), all_images
        )
        ...
        # a user with no permission can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.useless_user, ["change", "delete"]), no_images
        )
        ...
        # a user with permission, but not the matching one, can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.image_adder, ["change", "delete"]), no_images
        )
...
class TestOwnershipPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_instances_user_has_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.image_adder, "change"), [self.adder_image]
        )
        # a user with 'add' permission can also delete their own images
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.image_adder, "delete"), [self.adder_image]
        )
