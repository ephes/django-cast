...
class TestPageQuerySet(TestCase):
    ...
    def test_type_with_multiple_models(self):
        ...
        for page in pages:
            self.assertTrue(isinstance(page.specific, (EventPage, SimplePage)))
    ...
    def test_not_type_with_multiple_models(self):
        ...
        for page in pages:
            self.assertFalse(isinstance(page.specific, (EventPage, SimplePage)))
...
class TestPageQuerySetSearch(TestCase):
    ...
    def test_operators(self):
        ...
        self.assertEqual(
            list(results), [Page.objects.get(url_path="/home/events/tentative-unpublished-event/").specific]
        )
...
class TestSpecificQuery(TestCase):
    ...
    def test_specific_query_with_search(self):
        ...
        pages = list(Page.objects.specific().live().in_menu().search(MATCH_ALL, backend="wagtail.search.backends.db"))
...
class TestFirstCommonAncestor(TestCase):
    ...
    def test_single_page_no_include_self(self):
        ...
        self.assertEqual(
            Page.objects.get(slug="events"), Page.objects.filter(title="Christmas").first_common_ancestor()
        )
    ...
    def test_all_pages_include_self_strict(self):
        self.assertEqual(
            Page.get_first_root_node(), Page.objects.first_common_ancestor(include_self=True, strict=True)
        )
