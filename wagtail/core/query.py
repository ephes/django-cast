    ...
    def first_common_ancestor(self, include_self=False, strict=False):
        ...
        if include_self:
            ...
            paths = (
                ...
                .annotate(
                    parent_path=Substr(
                        "path", 1, Length("path") - self.model.steplen, output_field=CharField(max_length=255)
                    )
                )
    ...
    def annotate_approved_schedule(self):
        ...
        return self.annotate(
            _approved_schedule=Exists(
                PageRevision.objects.exclude(approved_go_live_at__isnull=True).filter(page__pk=OuterRef("pk"))
            )
        )
