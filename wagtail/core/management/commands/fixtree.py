...
class Command(BaseCommand):
    ...
    def handle_model(self, model, model_name, model_name_plural, any_problems_fixed, options):
        ...
        if orphans:
            ...
            if delete_orphans:
                ...
                self.stdout.write(
                    ...
                    % (deletion_count, model_name_plural if deletion_count != 1 else model_name)
                )
        ...
        if any((bad_alpha, bad_path, orphans, bad_depth, bad_numchild)):
            ...
            if orphans:
                self.stdout.write(f"Orphaned {model_name_plural} found: {self.numberlist_to_string(orphans)}")
            if bad_depth:
                self.stdout.write(
                    f"Incorrect depth value found for {model_name_plural}: {self.numberlist_to_string(bad_depth)}"
                )
