...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            "--dryrun", action="store_true", dest="dryrun", default=False, help="Dry run -- don't change anything."
        )
        ...
        if dryrun:
            if expired_pages:
                ...
                for ep in expired_pages:
                    self.stdout.write("{}\t{}\t{}".format(ep.expire_at.strftime("%Y-%m-%d %H:%M"), ep.slug, ep.title))
            else:
