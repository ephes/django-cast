...
if getattr(settings, "WAGTAIL_WORKFLOW_ENABLED", True):
    ...
    def register_workflow_permissions():
        return Permission.objects.filter(
            content_type__app_label="wagtailcore", codename__in=["add_workflow", "change_workflow", "delete_workflow"]
        )

    @hooks.register("register_permissions")
    def register_task_permissions():
        return Permission.objects.filter(
            content_type__app_label="wagtailcore", codename__in=["add_task", "change_task", "delete_task"]
        )
...
def describe_collection_children(collection):
    ...
    if descendant_count:
        ...
        return {
            ...
            "count_text": ngettext(
                "%(count)s descendant collection", "%(count)s descendant collections", descendant_count
            )
            % {"count": descendant_count},
            "url": url,
        }
...
def register_core_log_actions(actions):
    ...
    actions.register_action(
        "wagtail.unpublish.scheduled", _("Unpublish scheduled draft"), _("Unpublished scheduled draft")
    )
            ...
            try:
                return _('Added a comment on field %(field)s: "%(text)s"') % {
                    "field": self._field_label_from_content_path(
                        log_entry.page.specific_class, log_entry.data["comment"]["contentpath"]
                    ),
