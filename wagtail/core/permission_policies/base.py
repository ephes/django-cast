...
class ModelPermissionPolicy(BaseDjangoAuthPermissionPolicy):
    ...
    def users_with_any_permission(self, actions):
        permission_codenames = [f"{action}_{self.model_name}" for action in actions]
    ...
    * ability to edit also implies ability to delete
    ...
    def __init__(self, model, auth_model=None, owner_field_name="owner"):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                ...
                "you must specify a valid field name as owner_field_name." % (self.model, self.owner_field_name)
            )
    ...
    def users_with_any_permission(self, actions):
        if "change" in actions or "delete" in actions:
            ...
            permission_codenames = ["add_%s" % self.model_name, "change_%s" % self.model_name]
        elif "add" in actions:
