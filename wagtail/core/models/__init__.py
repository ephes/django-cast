...
class PageBase(models.base.ModelBase):
    ...
    def __init__(cls, name, bases, dct):
        ...
        if "template" not in dct:
            ...
            cls.template = f"{cls._meta.app_label}/{camelcase_to_underscore(name)}.html"
...
class Page(AbstractPage, index.Indexed, ClusterableModel, metaclass=PageBase):
    ...
    def save(self, clean=True, user=None, log_action=False, **kwargs):
        ...
        if log_action is not None:
            ...
            if is_new:
                ...
                PageLogEntry.objects.log_action(instance=self, action=log_action, user=user)
    ...
    def check(cls, **kwargs):
        ...
        try:
            ...
            errors.append(
                checks.Error("Invalid subpage_types setting for %s" % cls, hint=str(e), id="wagtailcore.E002")
            )
        ...
        try:
            ...
            errors.append(
                checks.Error("Invalid parent_page_types setting for %s" % cls, hint=str(e), id="wagtailcore.E002")
            )
    ...
    def update_aliases(self, *, revision=None, user=None, _content_json=None, _updated_ids=None):
        ...
        for alias in self.specific_class.objects.filter(alias_of=self).exclude(id__in=_updated_ids):
            ...
            page_published.send(
                sender=alias_updated.specific_class, instance=alias_updated, revision=revision, alias=True
            )
    ...
    def serve(self, request, *args, **kwargs):
        ...
        return TemplateResponse(
            request, self.get_template(request, *args, **kwargs), self.get_context(request, *args, **kwargs)
        )
    ...
    def move(self, target, pos=None, user=None):
        ...
        PageLogEntry.objects.log_action(
            ...
            user=user,
            data={
                "source": {"id": parent_before.id, "title": parent_before.specific_deferred.get_admin_display_title()},
                "destination": {
                    "id": parent_after.id,
    ...
    def copy(
        ...
        page_copy, child_object_map = _copy(
            specific_self, exclude_fields=exclude_fields, update_attrs=base_update_attrs
        )
        ...
        if page_copy.live:
            page_published.send(
                sender=page_copy.specific_class, instance=page_copy, revision=latest_revision_as_page_revision
            )
            ...
            PageLogEntry.objects.log_action(
                ...
                data={
                    "page": {
                        ...
                        "locale": {"id": page_copy.locale_id, "language_code": page_copy.locale.language_code},
                    },
                    "source": {"id": parent.id, "title": parent.specific_deferred.get_admin_display_title()}
                    if parent
                    else None,
                    "destination": {"id": to.id, "title": to.specific_deferred.get_admin_display_title()}
                    if to
                    else None,
                    "keep_live": page_copy.live and keep_live,
                    "source_locale": {"id": self.locale_id, "language_code": self.locale.language_code},
                },
            )
        ...
        if log_action:
            ...
            PageLogEntry.objects.log_action(
                ...
                data={
                    ...
                    "destination": {"id": parent.id, "title": parent.specific_deferred.get_admin_display_title()}
                    if parent
                    else None,
                },
            )
...
class GroupPagePermission(models.Model):
    group = models.ForeignKey(
        Group, verbose_name=_("group"), related_name="page_permissions", on_delete=models.CASCADE
    )
    page = models.ForeignKey(
        "Page", verbose_name=_("page"), related_name="group_permissions", on_delete=models.CASCADE
    )
    permission_type = models.CharField(
        verbose_name=_("permission type"), max_length=20, choices=PAGE_PERMISSION_TYPE_CHOICES
    )
...
class WorkflowPage(models.Model):
    page = models.OneToOneField(
        "Page", verbose_name=_("page"), on_delete=models.CASCADE, primary_key=True, unique=True
    )
...
class WorkflowTask(Orderable):
    workflow = ParentalKey(
        "Workflow", on_delete=models.CASCADE, verbose_name=_("workflow_tasks"), related_name="workflow_tasks"
    )
...
class Task(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, verbose_name=_("content type"), related_name="wagtail_tasks", on_delete=models.CASCADE
    )
...
class GroupApprovalTask(Task):
    groups = models.ManyToManyField(
        ...
        help_text=_("Pages at this step in a workflow will be moderated or approved by these groups of users"),
    )
...
class WorkflowState(models.Model):
    ...
    workflow = models.ForeignKey(
        "Workflow", on_delete=models.CASCADE, verbose_name=_("workflow"), related_name="workflow_states"
    )
    status = models.fields.CharField(
        choices=STATUS_CHOICES, verbose_name=_("status"), max_length=50, default=STATUS_IN_PROGRESS
    )
    ...
    current_task_state = models.OneToOneField(
        "TaskState", on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_("current task state")
    )

    # allows a custom function to be called on finishing the Workflow successfully.
    on_finish = import_string(
        getattr(settings, "WAGTAIL_FINISH_WORKFLOW_ACTION", "wagtail.core.workflows.publish_workflow_state")
    )
    ...
    def user_can_cancel(self, user):
        ...
        return (
            ...
            or (
                ...
                and "approve" in [action[0] for action in self.current_task_state.task.get_actions(self.page, user)]
            )
        )
    ...
    def revisions(self):
        ...
        return PageRevision.objects.filter(
            page_id=self.page_id, id__in=self.task_states.values_list("page_revision_id", flat=True)
        ).defer("content_json")
...
class TaskState(models.Model):
    ...
    workflow_state = models.ForeignKey(
        "WorkflowState", on_delete=models.CASCADE, verbose_name=_("workflow state"), related_name="task_states"
    )
    page_revision = models.ForeignKey(
        "PageRevision", on_delete=models.CASCADE, verbose_name=_("page revision"), related_name="task_states"
    )
    ...
    content_type = models.ForeignKey(
        ContentType, verbose_name=_("content type"), related_name="wagtail_task_states", on_delete=models.CASCADE
    )
    exclude_fields_in_copy = []
...
class Comment(ClusterableModel):
    ...
    revision_created = models.ForeignKey(
        PageRevision, on_delete=models.CASCADE, related_name="created_comments", null=True, blank=True
    )

    resolved_at = models.DateTimeField(null=True, blank=True)
    resolved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, related_name="comments_resolved", null=True, blank=True
    )
    ...
    def __str__(self):
        return f"Comment on Page '{self.page}', left by {self.user}: '{self.text}'"
