...
class BaseChooseView(View):
    def get(self, request):
        ...
        images = permission_policy.instances_user_has_any_permission_for(request.user, ["choose"]).order_by(
            "-created_at"
        )
...
def image_chosen(request, image_id):
    ...
    return render_modal_workflow(
        request, None, None, None, json_data={"step": "image_chosen", "result": get_image_result_data(image)}
    )
...
def chooser_upload(request):
    ...
    if request.method == "POST":
        ...
        form = ImageForm(request.POST, request.FILES, instance=image, user=request.user, prefix="image-chooser-upload")
    ...
    return render_modal_workflow(
        request, None, None, None, json_data={"step": "reshow_upload_form", "htmlFragment": upload_form_html}
    )
...
def chooser_select_format(request, image_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render_modal_workflow(
                request, None, None, None, json_data={"step": "image_chosen", "result": image_data}
            )
    else:
