...
def edit(request, image_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                buttons=[messages.button(reverse("wagtailimages:edit", args=(image.id,)), _("Edit again"))],
            )
    ...
    if image.is_stored_locally():
        ...
        if not os.path.isfile(image.file.path):
            messages.error(
                request,
                _("The source image file could not be found. Please change the source or delete the image.").format(
                    image.title
                ),
