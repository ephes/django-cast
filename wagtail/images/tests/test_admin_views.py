...
class TestImageIndexView(TestCase, WagtailTestUtils):
    ...
    def test_pagination_preserves_other_params(self):
        ...
        for i in range(1, 50):
            self.image = Image.objects.create(
                title="Test image %i" % i, file=get_test_image_file(size=(1, 1)), collection=evil_plans_collection
            )
    ...
    def test_tag_filtering_preserves_other_params(self):
        ...
        self.assertTrue("?p=2&amp;tag=even" in response_body or "?tag=even&amp;p=1" in response_body)
...
class TestImageAddView(TestCase, WagtailTestUtils):
    ...
    def test_add_too_many_pixels(self):
        ...
        # The form should have an error
        self.assertFormError(response, "form", "file", "This file has too many pixels (307200). Maximum pixels 1.")
    ...
    def test_unique_together_validation_error(self):
        ...
        CustomImage.objects.create(title="Test image", file=get_test_image_file(), collection=evil_plans_collection)
...
class TestImageChooserUploadView(TestCase, WagtailTestUtils):
    ...
    def test_select_format_flag_after_upload_form_error_bad_extension(self):
        ...
        self.assertTemplateUsed(response, "wagtailimages/chooser/upload_form.html")
        self.assertFormError(
            response, "form", "file", "Not a supported image format. Supported formats: GIF, JPEG, PNG, WEBP."
        )
...
class TestMultipleImageUploader(TestCase, WagtailTestUtils):
    ...
    def test_add_post(self):
        ...
        self.assertEqual(
            response.context["delete_action"], "/admin/images/multiple/%d/delete/" % response.context["image"].id
        )
...
class TestMultipleImageUploaderWithCustomImageModel(TestCase, WagtailTestUtils):
    ...
    def test_edit_fails_unique_together_validation(self):
        ...
        CustomImage.objects.create(title="The Eiffel Tower", file=get_test_image_file(), collection=new_collection)
...
class TestMultipleImageUploaderWithCustomRequiredFields(TestCase, WagtailTestUtils):
    ...
    def test_create_from_upload_invalid_post(self):
        ...
        # Send request
        response = self.client.post(
            reverse("wagtailimages:create_multiple_from_uploaded_image", args=(self.uploaded_image.id,)),
            {
...
class TestGenerateURLView(TestCase, WagtailTestUtils):
    ...
    def test_get(self):
        ...
        expected_url = "http://localhost/images/%(signature)s/%(image_id)d/fill-800x600/" % {
            "signature": quote(generate_signature(self.image.id, "fill-800x600"), safe=urlquote_safechars),
