...
class TestFormat(TestCase, WagtailTestUtils):
    ...
    def test_image_to_html_no_classnames(self):
        ...
        self.assertTagInHTML('<img width="640" height="480" alt="test alt text">', result, allow_extra_attrs=True)
...
class TestFrontendServeView(TestCase):
    ...
    def test_get_with_serve_action(self):
        signature = generate_signature(self.image.id, "fill-800x600")
        response = self.client.get(
            reverse("wagtailimages_serve_action_serve", args=(signature, self.image.id, "fill-800x600"))
        )
    ...
    def test_get_with_redirect_action(self):
        signature = generate_signature(self.image.id, "fill-800x600")
        response = self.client.get(
            reverse("wagtailimages_serve_action_redirect", args=(signature, self.image.id, "fill-800x600"))
        )
    ...
    def test_get_with_custom_key(self):
        ...
        # Get the image
        response = self.client.get(
            reverse("wagtailimages_serve_custom_key", args=(signature, self.image.id, "fill-800x600")) + "test.png"
        )
...
class TestFrontendSendfileView(TestCase):
    ...
    def test_sendfile_nobackend(self):
        signature = generate_signature(self.image.id, "fill-800x600")
        response = self.client.get(reverse("wagtailimages_sendfile", args=(signature, self.image.id, "fill-800x600")))
    ...
    def test_sendfile_dummy_backend(self):
        signature = generate_signature(self.image.id, "fill-800x600")
        response = self.client.get(
            reverse("wagtailimages_sendfile_dummy", args=(signature, self.image.id, "fill-800x600"))
        )
