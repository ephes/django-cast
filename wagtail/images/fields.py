...
class WagtailImageField(ImageField):
    def __init__(self, *args, **kwargs):
        ...
        self.error_messages["invalid_image_extension"] = (
            _("Not a supported image format. Supported formats: %s.") % SUPPORTED_FORMATS_TEXT
        )
        ...
        self.error_messages["file_too_many_pixels"] = (
            _("This file has too many pixels (%%s). Maximum pixels %s.") % self.max_image_pixels
        )
    ...
    def check_image_file_format(self, f):
        ...
        if internal_image_format != image_format:
            raise ValidationError(
                self.error_messages["invalid_image_known_format"] % (image_format,), code="invalid_image_known_format"
            )
    ...
    def check_image_file_size(self, f):
        ...
        if f.size > self.max_upload_size:
            raise ValidationError(
                self.error_messages["file_too_large"] % (filesizeformat(f.size),), code="file_too_large"
            )
    ...
    def check_image_pixel_size(self, f):
        ...
        if num_pixels > self.max_image_pixels:
            raise ValidationError(
                self.error_messages["file_too_many_pixels"] % (num_pixels), code="file_too_many_pixels"
            )
