...
class WagtailPageTests(WagtailTestUtils, TestCase):
    ...
    def assertCanCreateAt(self, parent_model, child_model, msg=None):
        ...
        if not self._testCanCreateAt(parent_model, child_model):
            msg = self._formatMessage(
                msg,
                "Can not create a {}.{} under a {}.{}".format(
    ...
    def assertCanCreate(self, parent, child_model, data, msg=None):
        ...
        add_url = reverse(
            "wagtailadmin_pages:add", args=[child_model._meta.app_label, child_model._meta.model_name, parent.pk]
        )
        ...
        if response.redirect_chain == []:
            ...
            msg = self._formatMessage(
                msg,
                "Validation errors found when creating a {}.{}:\n{}".format(
        ...
        if response.redirect_chain != [(explore_url, 302)]:
            msg = self._formatMessage(
                msg,
                "Creating a page {}.{} didn't redirect the user to the explorer, but to {}".format(
                    child_model._meta.app_label, child_model._meta.model_name, response.redirect_chain
                ),
            )
            raise self.failureException(msg)
