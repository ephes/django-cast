...
from wagtail.core.blocks import CharBlock, FieldBlock, RawHTMLBlock, RichTextBlock, StreamBlock, StructBlock
...
class LinkFields(models.Model):
    ...
    link_document = models.ForeignKey(
        "wagtaildocs.Document", null=True, blank=True, related_name="+", on_delete=models.CASCADE
    )
...
class EventPageSpeaker(TranslatableMixin, Orderable, LinkFields, ClusterableModel):
    page = ParentalKey(
        "tests.EventPage", related_name="speakers", related_query_name="speaker", on_delete=models.CASCADE
    )
...
class SingleEventPage(EventPage):
    excerpt = models.TextField(
        max_length=255, blank=True, null=True, help_text="Short text to describe what is this action about"
    )
...
class EventIndex(Page):
    ...
    def get_static_site_paths(self):
        ...
        yield from super().get_static_site_paths()

    def get_sitemap_urls(self, request=None):
        # Add past events url to sitemap
        return super().get_sitemap_urls(request=request) + [
            {"location": self.full_url + "past/", "lastmod": self.latest_revision_created_at}
        ]
...
class FormPageWithCustomSubmission(AbstractEmailForm):
    ...
    def process_form_submission(self, form):
        form_submission = self.get_submission_class().objects.create(
            form_data=json.dumps(form.cleaned_data, cls=DjangoJSONEncoder), page=self, user=form.user
        )
...
class FormFieldForCustomListViewPage(AbstractFormField):
    page = ParentalKey("FormPageWithCustomSubmissionListView", related_name="form_fields", on_delete=models.CASCADE)
...
class ExtendedFormField(AbstractFormField):
    ...
    page = ParentalKey("FormPageWithCustomFormBuilder", related_name="form_fields", on_delete=models.CASCADE)
...
class CustomDocument(AbstractDocument):
    ...
    admin_form_fields = Document.admin_form_fields + ("description", "fancy_description")
...
class SectionedRichTextPage(Page):
    content_panels = [FieldPanel("title", classname="full title"), InlinePanel("sections")]
...
class PerUserContentPanels(ObjectList):
    def _replace_children_with_per_user_config(self):
        ...
        self.children = [
            child.bind_to(model=self.model, instance=self.instance, request=self.request, form=self.form)
...
class PerUserPageMixin:
    ...
    def get_edit_handler(cls):
        ...
        if cls.settings_panels:
            tabs.append(ObjectList(cls.settings_panels, heading="Settings", classname="settings"))
