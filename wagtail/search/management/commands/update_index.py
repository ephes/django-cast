...
class Command(BaseCommand):
    def update_backend(self, backend_name, schema_only=False, chunk_size=DEFAULT_CHUNK_SIZE):
        ...
        for index, models in models_grouped_by_index:
            ...
            if not schema_only:
                for model in models:
                    self.stdout.write(
                        f"{backend_name}: {model._meta.app_label}.{model.__name__} ".ljust(35), ending=""
                    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--backend", action="store", dest="backend_name", default=None, help="Specify a backend to update"
        )
        parser.add_argument(
            "--schema-only",
            action="store_true",
    ...
    def handle(self, **options):
        ...
        for backend_name in backend_names:
            self.update_backend(
                backend_name, schema_only=options.get("schema_only", False), chunk_size=options.get("chunk_size")
            )
