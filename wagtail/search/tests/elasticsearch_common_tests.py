...
class ElasticsearchCommonSearchBackendTests(BackendTests):
    ...
    def test_child_partial_search(self):
        ...
        self.assertUnsortedListEqual(
            [r.title for r in results], ["A Game of Thrones", "A Storm of Swords", "A Clash of Kings"]
        )
    ...
    def test_search_with_hyphen(self):
        ...
        book = models.Book.objects.create(
            title="Harry Potter and the Half-Blood Prince", publication_date=date(2009, 7, 15), number_of_pages=607
        )
    ...
    def test_more_than_one_hundred_results(self):
        ...
        for i in range(150):
            books.append(
                models.Book.objects.create(title=f"Book {i}", publication_date=date(2017, 10, 21), number_of_pages=i)
            )
