...
class TestGarbageCollectCommand(TestCase):
    def test_garbage_collect_command(self):
        ...
        promoted_querie_ids = []
        for i in range(10):
            q = models.Query.get(f"Foo bar {i}")
            q.add_hit(date=old_hit_date)
        ...
        self.assertEqual(
            models.QueryDailyHits.objects.filter(date=recent_hit_date, query_id__in=recent_querie_ids).count(), 10
        )

        self.assertEqual(models.Query.objects.filter(id__in=promoted_querie_ids).count(), 10)
        self.assertEqual(
            models.QueryDailyHits.objects.filter(date=recent_hit_date, query_id__in=promoted_querie_ids).count(), 0
        )
