...
class TestFormResponsesPanel(TestCase):
    def setUp(self):
        ...
        self.FormPageForm = get_form_for_model(FormPage, form_class=WagtailAdminPageForm)
...
class TestFormResponsesPanelWithCustomSubmissionClass(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.FormPageForm = get_form_for_model(FormPageWithCustomSubmission, form_class=WagtailAdminPageForm)
...
class TestFormsSubmissionsList(TestCase, WagtailTestUtils):
    ...
    def make_list_submissions(self):
        ...
        """
        for i in range(100):
            submission = FormSubmission(page=self.form_page, form_data=json.dumps({"hello": "world"}))
            submission.save()
    ...
    def test_list_submissions_url_params_ordering_recent_first(self):
        response = self.client.get(
            reverse("wagtailforms:list_submissions", args=(self.form_page.id,)), {"order_by": "-submit_time"}
        )
...
class TestFormsSubmissionsListLegacyFieldName(TestCase, WagtailTestUtils):
    ...
    def setUp(self):
        ...
        self.assertEqual(messages, [Info("Added `clean_name` on 3 form field(s)", obj=FormFieldWithCustomSubmission)])

        # check clean_name has been updated
        self.assertEqual(FormFieldWithCustomSubmission.objects.all()[0].clean_name, "your-email")
...
class TestFormsSubmissionsExport(TestCase, WagtailTestUtils):
    ...
    def test_list_submissions_csv_export(self):
        response = self.client.get(
            reverse("wagtailforms:list_submissions", args=(self.form_page.id,)), {"export": "csv"}
        )
        ...
        if settings.USE_TZ:
            self.assertEqual(
                data_lines[1], '2013-01-01 12:00:00+00:00,old@example.com,this is a really old message,"foo, baz"\r'
            )
            self.assertEqual(
                data_lines[2], "2014-01-01 12:00:00+00:00,new@example.com,this is a fairly new message,None\r"
            )
        else:
            self.assertEqual(
                data_lines[1], '2013-01-01 12:00:00,old@example.com,this is a really old message,"foo, baz"\r'
            )
    ...
    def test_list_submissions_xlsx_export(self):
        response = self.client.get(
            reverse("wagtailforms:list_submissions", args=(self.form_page.id,)), {"export": "xlsx"}
        )
        ...
        self.assertEqual(
            cell_array[1],
            [datetime.datetime(2013, 1, 1, 12, 0), "old@example.com", "this is a really old message", "foo, baz"],
        )
        self.assertEqual(
            cell_array[2],
            [datetime.datetime(2014, 1, 1, 12, 0), "new@example.com", "this is a fairly new message", "None"],
        )
    ...
    def test_list_submissions_csv_export_with_date_from_filtering(self):
        ...
        if settings.USE_TZ:
            self.assertEqual(
                data_lines[1], "2014-01-01 12:00:00+00:00,new@example.com,this is a fairly new message,None\r"
            )
        else:
...
class TestFormsWithCustomSubmissionsList(TestCase, WagtailTestUtils):
    ...
    def test_list_submissions_csv_export(self):
        ...
        self.assertEqual(
            data_lines[0], "User email,Submission date,Your email,Chocolate,Ingredients,Your Excitement\r"
        )
