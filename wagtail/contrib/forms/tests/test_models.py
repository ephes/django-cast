...
class TestFormSubmission(TestCase):
    ...
    def test_label_escaping(self):
        ...
        response = self.client.get("/contact-us/")
        self.assertContains(
            response, """<label for="id_your_message">Your &lt;em&gt;wonderful&lt;/em&gt; message</label>"""
        )

    def test_post_invalid_form(self):
        response = self.client.post(
            "/contact-us/", {"your_email": "bob", "your_message": "hello world", "your_choices": ""}
        )
    ...
    def test_post_multiple_values(self):
        ...
        form_page = Page.objects.get(url_path="/home/contact-us/")
        submission = FormSubmission.objects.filter(page=form_page, form_data__contains="hello world")
...
class TestFormWithCustomSubmission(TestCase, WagtailTestUtils):
    ...
    def test_post_multiple_values(self):
        ...
        # Check that the three checkbox values were saved correctly
        form_page = Page.objects.get(url_path="/home/contact-us/")
        submission = CustomFormPageSubmission.objects.filter(page=form_page, form_data__contains="hello world")
        self.assertIn("foo", submission[0].form_data)
        self.assertIn("bar", submission[0].form_data)
        self.assertIn("baz", submission[0].form_data)
...
class TestFormPageWithCustomFormBuilder(TestCase, WagtailTestUtils):
    ...
    def test_get_form(self):
        ...
        self.assertContains(
            response, '<input type="text" name="name" required maxlength="120" id="id_name" />', html=True
        )
        # check ip address field has rendered
        self.assertContains(
            response, '<input type="text" name="device_ip_address" required id="id_device_ip_address" />', html=True
        )
