...
def start_import(request):
    ...
    if request.POST or request.FILES:
        form_kwargs = {}
        form = ImportForm(supported_extensions, request.POST or None, request.FILES or None, **form_kwargs)
    else:
    ...
    if extension not in supported_extensions:
        messages.error(request, _('File format of type "{}" is not supported').format(extension))
...
def process_import(request):
    ...
    form_kwargs = {}
    form = ConfirmImportForm(DEFAULT_FORMATS, request.POST or None, request.FILES or None, **form_kwargs)
    ...
    messages.success(
        request, ngettext("Imported %(total)d redirect", "Imported %(total)d redirects", total) % {"total": total}
    )
