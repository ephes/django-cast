...
class TestRedirects(TestCase):
    ...
    def test_path_normalisation(self):
        ...
        self.assertEqual(
            path, normalise_path("/Hello/world.html;fizz=three;buzz=five?foo=Bar&Baz=quux2")  # The exact same URL
        )
        self.assertEqual(
            path,
        ...
        self.assertEqual(
            path, normalise_path("  /Hello/world.html;fizz=three;buzz=five?foo=Bar&Baz=quux2")  # Leading whitespace
        )
        self.assertEqual(
            path, normalise_path("/Hello/world.html;fizz=three;buzz=five?foo=Bar&Baz=quux2  ")  # Trailing whitespace
        )

        # Test against different paths
        self.assertNotEqual(
            path, normalise_path("/hello/world.html;fizz=three;buzz=five?foo=Bar&Baz=quux2")  # 'hello' is lowercase
        )
        ...
        self.assertNotEqual(
            path, normalise_path("/Hello/WORLD.html;fizz=three;buzz=five?foo=Bar&Baz=quux2")  # 'WORLD' is uppercase
        )
        self.assertNotEqual(
            path,
    ...
    def test_unicode_path_normalisation(self):
        ...
        self.assertEqual("/here/tésting-ünicode", normalise_path("/here/tésting-ünicode"))  # stays the same
    ...
    def test_redirect_from_any_site(self):
        ...
        self.assertRedirects(
            response, "http://localhost/events/christmas/", status_code=301, fetch_redirect_response=False
        )
    ...
    def test_duplicate_redirects_with_query_string_when_match_is_for_generic(self):
        ...
        models.Redirect.objects.create(
            site=site, old_path="/xmas?foo=Bar", redirect_link="/site-specific-with-query-string"
        )
    ...
    def test_duplicate_redirects_with_query_string_when_match_is_for_specific_with_qs(self):
        ...
        self.assertRedirects(
            response, "/site-specific-with-query-string", status_code=301, fetch_redirect_response=False
        )
