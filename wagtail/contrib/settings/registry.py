...
class SettingMenuItem(MenuItem):
    def __init__(self, model, icon="cog", classnames="", **kwargs):
        ...
        super().__init__(
            ...
            url=reverse("wagtailsettings:edit", args=[model._meta.app_label, model._meta.model_name]),
...
class SettingsAdminURLFinder(ModelAdminURLFinder):
    def construct_edit_url(self, instance):
        return reverse(
            "wagtailsettings:edit", args=[self.model._meta.app_label, self.model._meta.model_name, instance.site_id]
        )
    ...
    def register(self, model, **kwargs):
        ...
        def permissions_hook():
            return Permission.objects.filter(
                content_type__app_label=model._meta.app_label, codename=f"change_{model._meta.model_name}"
            )
