...
class TestContextProcessor(TemplateTestCase):
    def test_accessing_setting(self):
        ...
        self.assertEqual(
            self.render(request, "{{ settings.tests.TestSetting.title }}"), self.default_site_settings.title
        )
    ...
    def test_multisite(self):
        ...
        self.assertEqual(
            self.render(request, "{{ settings.tests.TestSetting.title }}"), self.other_site_settings.title
        )
        ...
        with self.assertNumQueries(1):
            for i in range(1, 4):
                with self.subTest(attempt=i):
                    self.assertEqual(self.render(request, get_title * i), self.default_site_settings.title * i)
...
class TestTemplateTag(TemplateTestCase):
    ...
    def test_get_settings_request_context(self):
        ...
        template = Template(
            "{% load wagtailsettings_tags %}" "{% get_settings %}" "{{ settings.tests.testsetting.title}}"
        )
...
class TestSettingsJinja(TemplateTestCase):
    ...
    def test_accessing_setting(self):
        """Check that the context processor works"""
        self.assertEqual(self.render('{{ settings("tests.TestSetting").title }}'), self.default_site_settings.title)

    def test_multisite(self):
        """Check that the correct setting for the current site is returned"""
        context = {"site": self.default_site}
        self.assertEqual(
            self.render('{{ settings("tests.TestSetting").title }}', context), self.default_site_settings.title
        )

        context = {"site": self.other_site}
        self.assertEqual(
            self.render('{{ settings("tests.TestSetting").title }}', context), self.other_site_settings.title
        )
    ...
    def test_models_cached(self):
        ...
        for i in range(1, 4):
            with self.assertNumQueries(1):
                ...
                self.assertEqual(template.render(context), self.default_site_settings.title * i)
    ...
    def test_settings_use_default_site_override(self):
        ...
        self.assertEqual(self.render(template, context), self.default_site_settings.title)
    ...
    def test_settings_use_default_site(self):
        ...
        self.assertEqual(self.render(template, context, request_context=False), self.default_site_settings.title)
