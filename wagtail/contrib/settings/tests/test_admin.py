...
from wagtail.tests.testapp.models import FileUploadSetting, IconSetting, PanelSettings, TabbedSettings, TestSetting
...
class TestSettingCreateView(BaseTestSettingView):
    ...
    def test_edit_invalid(self):
        ...
        self.assertContains(
            response, """<p class="error-message"><span>This field is required.</span></p>""", count=2, html=True
        )
...
class TestMultiSite(BaseTestSettingView):
    ...
    def test_redirect_to_default(self):
        ...
        dest_url = reverse("wagtailsettings:edit", args=["tests", "testsetting", self.default_site.pk])
    ...
    def test_redirect_to_current(self):
        ...
        """
        start_url = reverse("wagtailsettings:edit", args=["tests", "testsetting"])
        dest_url = reverse("wagtailsettings:edit", args=["tests", "testsetting", self.other_site.pk])
    ...
    def test_edit(self):
        ...
        """
        TestSetting.objects.create(title="default", email="default@example.com", site=self.default_site)
        TestSetting.objects.create(title="other", email="other@example.com", site=self.other_site)
        response = self.post(
            site_pk=self.other_site.pk, post_data={"title": "other-new", "email": "other-other@example.com"}
        )
        self.assertEqual(response.status_code, 302)
