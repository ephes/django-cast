...
class AdminURLHelper:
    ...
    def _get_action_url_pattern(self, action):
        if action == "index":
            return fr"^{self.opts.app_label}/{self.opts.model_name}/$"
        return fr"^{self.opts.app_label}/{self.opts.model_name}/{action}/$"

    def _get_object_specific_action_url_pattern(self, action):
        return fr"^{self.opts.app_label}/{self.opts.model_name}/{action}/(?P<instance_pk>[-\w]+)/$"
    ...
    def get_action_url_name(self, action):
        return f"{self.opts.app_label}_{self.opts.model_name}_modeladmin_{action}"
