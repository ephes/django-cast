...
class ModelFormView(WMABaseView, FormView):
    ...
    def get_edit_handler(self):
        ...
        edit_handler = self.model_admin.get_edit_handler(instance=instance, request=self.request)
        return edit_handler.bind_to(model=self.model_admin.model, request=self.request, instance=instance)
    ...
    def media(self):
        return forms.Media(
            css={"all": self.model_admin.get_form_view_extra_css()}, js=self.model_admin.get_form_view_extra_js()
        )
    ...
    def form_valid(self, form):
        instance = form.save()
        messages.success(
            self.request, self.get_success_message(instance), buttons=self.get_success_message_buttons(instance)
        )
...
class IndexView(SpreadsheetExportMixin, WMABaseView):
    ...
    def media(self):
        return forms.Media(
            css={"all": self.model_admin.get_index_view_extra_css()}, js=self.model_admin.get_index_view_extra_js()
        )

    def get_buttons_for_obj(self, obj):
        return self.button_helper.get_buttons_for_obj(obj, classnames_add=["button-small", "button-secondary"])
...
class InspectView(InstanceSpecificView):
    ...
    def media(self):
        return forms.Media(
            css={"all": self.model_admin.get_inspect_view_extra_css()}, js=self.model_admin.get_inspect_view_extra_js()
        )
    ...
    def get_document_field_display(self, field_name, field):
        ...
        if document:
            return mark_safe(
                '<a href="{}">{} <span class="meta">({}, {})</span></a>'.format(
                    document.url,
                    document.title,
