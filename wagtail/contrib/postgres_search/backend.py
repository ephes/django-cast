...
class ObjectIndexer:
    ...
    def prepare_field(self, obj, field):
        if isinstance(field, SearchField):
            yield (field, get_weight(field.boost), self.prepare_value(field.get_value(obj)))
...
class Index:
    ...
    def add_items_upsert(self, content_type_pk, indexers):
        ...
        data_sql = ", ".join(
            [f"(%s, %s, {a}, {b}, {c}, 1.0)" for a, b, c in zip(title_sql, autocomplete_sql, body_sql)]
        )

        with self.connection.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO {} (content_type_id, object_id, title, autocomplete, body, title_norm)
                (VALUES {})
...
class PostgresSearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def build_tsrank(self, vector, query, config=None, boost=1.0):
        if isinstance(query, (Phrase, PlainText, Not)):
            rank_expression = SearchRank(vector, self.build_tsquery(query, config=config), weights=self.sql_weights)
...
class PostgresSearchResults(BaseSearchResults):
    def get_queryset(self, for_count=False):
        ...
        return self.query_compiler.search(
            self.query_compiler.get_config(self.backend), start, stop, score_field=self._score_field
        )
