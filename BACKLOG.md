...
- [ ] Evaluate Wagtail v3 API reuse for programmatic authoring
  ...
  - Progress: 0.2.66 adds test-only Wagtail 8 mounting, discovery, scalar draft-write,
    ...
    scheduling-input, and draft read/preview experiments. The adapter resolves measured response and latest-draft gaps; v3 actions reuse Cast's audio
    ...
    explore-only users read drafts, and has no rendered preview. Body, media, and PostgreSQL concurrency remain to be
    evaluated.
