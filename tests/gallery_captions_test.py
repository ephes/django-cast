...
@pytest.mark.django_db
@pytest.mark.parametrize("use_repository", [True, False])
def test_gallery_context_preserves_custom_struct_value_without_mutating_original(image, use_repository):
    from wagtail.blocks import StructValue

    class CustomGalleryValue(StructValue):
        @property
        def layout_label(self):
            return f"Layout: {self['layout']}"

    block = GalleryBlockWithLayout(value_class=CustomGalleryValue)
    value = block.to_python(
        {"layout": "default", "gallery": [{"type": "item", "value": {"image": image.pk, "caption": "Kept"}}]}
    )
    assert isinstance(value, CustomGalleryValue)
    original_gallery = value["gallery"]
    original_bound_gallery = value.bound_blocks["gallery"]
    repository = EmptyImageRepository()
    if use_repository:
        repository.image_by_id = {image.pk: image}

    context = block.get_context(value, {"repository": repository})
    resolved = context["value"]
    assert isinstance(resolved, CustomGalleryValue)
    assert resolved is not value
    assert resolved.block is block
    assert resolved.layout_label == "Layout: default"
    assert resolved.bound_blocks["gallery"].value is resolved["gallery"]
    assert resolved.bound_blocks["gallery"] is not original_bound_gallery
    assert resolved["gallery"] == [{"image": image, "caption": "Kept"}]
    assert value["gallery"] is original_gallery
    assert value.bound_blocks["gallery"] is original_bound_gallery
    assert block.get_prep_value(value)["gallery"][0]["value"] == {"image": image.pk, "caption": "Kept"}
