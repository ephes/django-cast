...
def test_delete_wagtail_images_app_setting(mocker):
    ...
    mocker.patch("cast.apps.appsettings.DELETE_WAGTAIL_IMAGES", True)
    mocked_disconnect = mocker.patch("cast.apps.post_delete.disconnect")
    cast_config.ready()
    assert mocked_disconnect.call_count == 0

    # delete images False -> disconnect post_delete_file_cleanup
    mocker.patch("cast.apps.appsettings.DELETE_WAGTAIL_IMAGES", False)
    mocked_disconnect = mocker.patch("cast.apps.post_delete.disconnect")
    cast_config.ready()
