...
class TestAllVideoEndpoints:
    ...
    def test_get_all_authenticated(self, admin_client, video_urls):
        for view_name, url in video_urls.urls.items():
...
class TestVideoIndex:
    ...
    def test_get_index(self, admin_client, video_urls):
        r = admin_client.get(video_urls.index)
    ...
    def test_get_index_ajax(self, admin_client, video_urls):
        headers = {"HTTP_X_REQUESTED_WITH": "XMLHttpRequest"}
        r = admin_client.get(video_urls.index, **headers)
    ...
    def test_get_index_with_search(self, admin_client, video_urls):
        r = admin_client.get(video_urls.index, {"q": video_urls.video.title})
    ...
    def test_get_index_with_search_invalid(self, admin_client, video_urls):
        r = admin_client.get(video_urls.index, {"q": " "})
...
class TestVideoAdd:
    ...
    def test_get_add_video(self, admin_client, video_urls):
        r = admin_client.get(video_urls.add)

        assert r.status_code == 200
        content = r.content.decode("utf-8")
        assert "Uploading…" in content

    def test_post_add_video_invalid_form(self, admin_client):
...
class TestVideoEdit:
    ...
    def test_get_edit_video(self, admin_client, video_urls):
        r = admin_client.get(video_urls.edit)

        assert r.status_code == 200
        content = r.content.decode("utf-8")
        assert "Delete" in content

    def test_get_edit_video_without_original(self, admin_client, video_without_original):
    ...
    def test_get_edit_video_with_original_no_filesize(self, settings, admin_client, video_without_file):
    ...
    def test_post_edit_video_invalid_form(self, admin_client, video_urls):
...
class TestVideoDelete:
    ...
    def test_get_delete_video(self, admin_client, video_urls):
        r = admin_client.get(video_urls.delete)

        assert r.status_code == 200
        content = r.content.decode("utf-8")
        assert "Are you sure you want to delete this video?" in content

    def test_post_delete_video(self, admin_client, video_urls):
        video = video_urls.video
        # post data is necessary because of if request.POST
        r = admin_client.post(video_urls.delete, {"delete": "yes"})
...
class TestVideoChosen:
    ...
    def test_get_chosen_video_not_found(self, admin_client, video_urls):
        video = video_urls.video
        video.delete()
        r = admin_client.get(video_urls.chosen)

        assert r.status_code == 404

    def test_get_chosen_video_success(self, admin_client, video_urls):
        video = video_urls.video
        r = admin_client.get(video_urls.chosen)
...
class TestVideoChooser:
    ...
    def test_get_video_in_chooser(self, admin_client, video_urls):
        video = video_urls.video
        r = admin_client.get(video_urls.chooser)
    ...
    def test_get_video_chooser_with_search(self, admin_client, video_urls):
        r = admin_client.get(video_urls.chooser, {"q": video_urls.video.title})
    ...
    def test_get_video_chooser_with_search_invalid(self, admin_client, video_urls):
        # {"p": "1"} (page 1) leads to the search form being invalid
        r = admin_client.get(video_urls.chooser, {"p": "1"})
...
class TestVideoChooserUpload:
    ...
    def test_get_video_in_chooser_upload(self, admin_client, video_urls):
        video = video_urls.video
        r = admin_client.get(video_urls.chooser_upload)
    ...
    def test_post_upload_video(self, admin_client, minimal_mp4):
        upload_url = reverse("castvideo:chooser_upload")
